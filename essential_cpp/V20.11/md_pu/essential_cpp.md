...
ポリモーフィックなクラスは、
仮想関数呼び出しを行う(「[オーバーライドとオーバーロードの違い](#SS_4_8_1)」参照)
ためのメモリレイアウトが必要になる。
...
各クラスがvtblへのポインタを保持するため、このドキュメントで使用している[g++](#SS_4_10_1)では、
...
このドキュメントのコードのコンパイルに使用している[g++](#SS_4_10_1)/[clang++](#SS_4_10_2)では、
...
これにより、[Static Initialization Order Fiasco(静的初期化順序問題)](#SS_4_9_11)を回避できる。
...
* [ゼロの原則(Rule of Zero)](#SS_4_5_1)
* [五の原則(Rule of Five)](#SS_4_5_2)

この2つの原則(ガイドライン)の使い分けに関しては、

* リソース管理を外部([RAII(scoped guard)](#SS_6_10)クラス)に任せられる場合: ゼロの法則を採用し、特殊メンバ関数を明示的に定義しない。
...
といった[セマンティクス](#SS_4_11_1)的観点によるものである。
...
このような状態のリファレンスを[danglingリファレンス](#SS_4_8_2)と呼ぶ。
同様に、このような状態のポインタを[danglingポインタ](#SS_4_8_3)と呼ぶ。
...
* C++11の[moveセマンティクス](#SS_4_3_3)と[perfect forwarding](#SS_2_8_5)を実現するために導入された。
...
この動作を理解することは、
[moveセマンティクス](#SS_4_3_3)や[perfect forwarding](#SS_2_8_5)を正しく実装/使用するために極めて重要である。
...
下記GetString0()のような関数が返すオブジェクトの内部メンバに対する[ハンドル](#SS_4_9_6)は、
...
サンプルコードを[g++](#SS_4_10_1)と[clang++](#SS_4_10_2)でコンパイルしている)。
...
によるとこの動作は未定義のようだが、
[g++](#SS_4_10_1)/[clang++](#SS_4_10_2)両方ともこのコードを警告なしでコンパイルする)。
...
このテストはエラーとならなければならないが、[g++](#SS_4_10_1)ではパスしてしまう。
2nd name lookupのロジックにバグがあるようである。

有難いことに、[clang++](#SS_4_10_2)では仕様通りこのテストはエラーとなり、
...
Tが[CopyAssignable要件](#SS_4_3_5)を満たすためには`std::is_copy_assignable<T>`がtrueでなければならないが、
その逆が成立するとは限らない。


### std::is_move_assignable <a id="SS_3_2_9"></a>
std::is_move_assignableはテンプレートパラメータの型(T)がmove代入可能かを調べる。
Tが[MoveAssignable要件](#SS_4_3_4)を満たすためには`std::is_move_assignable<T>`がtrueでなければならないが、
その逆が成立するとは限らない。
...
[クリティカルセクション](#SS_4_9_4)と呼ぶ)が、std::mutexによりこの問題を回避している。
...
一般に条件変数には、[Spurious Wakeup](#SS_4_9_10)という問題があり、std::condition_variableも同様である。

上記の抜粋である下記のコード例では[Spurious Wakeup](#SS_4_9_10)の対策が行われていないため、
意図通り動作しない可能性がある。
...
[オブジェクトの排他所有](#SS_4_2_1)を表すために用いられる。所有権は一つのunique_ptrインスタンスに限定され、
他のポインタと共有することはできない。ムーブ操作によってのみ所有権を移譲でき、
スコープを抜けると自動的にリソースが解放されるため、メモリ管理の安全性と効率性が向上する。

### std::shared_ptr <a id="SS_3_5_2"></a>
std::shared_ptrは、同じくC++11で導入されたスマートポインタであり、[オブジェクトの共有所有](#SS_4_2_2)を表すために用いられる。
...
その`shared_ptr`インスタンスとの関連のみを保持するのため、[オブジェクトの循環所有](#SS_4_2_3)の問題を解決できる。

[オブジェクトの循環所有](#SS_4_2_3)で示した問題のあるクラスの修正版を以下に示す
...
`std::auto_ptr`はC++11以前に導入された初期のスマートポインタであるが、異常な[copyセマンティクス](#SS_4_3_2)を持つため、
...
std::variantとstd::visit([Visitor](#SS_6_21)パターンの実装の一種)を組み合わせた場合の使用例を以下に示す。
...
&emsp;&emsp; [オブジェクト指向](#SS_4_1)  
&emsp;&emsp;&emsp; [is-a](#SS_4_1_1)  
&emsp;&emsp;&emsp; [has-a](#SS_4_1_2)  
&emsp;&emsp;&emsp; [is-implemented-in-terms-of](#SS_4_1_3)  
&emsp;&emsp;&emsp;&emsp; [public継承によるis-implemented-in-terms-of](#SS_4_1_3_1)  
&emsp;&emsp;&emsp;&emsp; [private継承によるis-implemented-in-terms-of](#SS_4_1_3_2)  
&emsp;&emsp;&emsp;&emsp; [コンポジションによる(has-a)is-implemented-in-terms-of](#SS_4_1_3_3)  

&emsp;&emsp; [オブジェクトの所有権](#SS_4_2)  
&emsp;&emsp;&emsp; [オブジェクトの排他所有](#SS_4_2_1)  
&emsp;&emsp;&emsp; [オブジェクトの共有所有](#SS_4_2_2)  
&emsp;&emsp;&emsp; [オブジェクトの循環所有](#SS_4_2_3)  

&emsp;&emsp; [copy/moveと等価性のセマンティクス](#SS_4_3)  
&emsp;&emsp;&emsp; [等価性のセマンティクス](#SS_4_3_1)  
&emsp;&emsp;&emsp; [copyセマンティクス](#SS_4_3_2)  
&emsp;&emsp;&emsp; [moveセマンティクス](#SS_4_3_3)  
&emsp;&emsp;&emsp; [MoveAssignable要件](#SS_4_3_4)  
...
&emsp;&emsp; [クラス設計のガイドライン](#SS_4_5)  
&emsp;&emsp;&emsp; [ゼロの原則(Rule of Zero)](#SS_4_5_1)  
&emsp;&emsp;&emsp; [五の原則(Rule of Five)](#SS_4_5_2)  
&emsp;&emsp;&emsp; [クラス凝集性のクライテリア](#SS_4_5_3)  

&emsp;&emsp; [コーディングスタイル](#SS_4_6)  
&emsp;&emsp;&emsp; [AAAスタイル](#SS_4_6_1)  
&emsp;&emsp;&emsp; [east-const](#SS_4_6_2)  
&emsp;&emsp;&emsp; [west-const](#SS_4_6_3)  

&emsp;&emsp; [オブジェクトのコピー](#SS_4_7)  
&emsp;&emsp;&emsp; [シャローコピー](#SS_4_7_1)  
&emsp;&emsp;&emsp; [ディープコピー](#SS_4_7_2)  
&emsp;&emsp;&emsp; [スライシング](#SS_4_7_3)  

&emsp;&emsp; [C++注意点](#SS_4_8)  
&emsp;&emsp;&emsp; [オーバーライドとオーバーロードの違い](#SS_4_8_1)  
&emsp;&emsp;&emsp; [danglingリファレンス](#SS_4_8_2)  
&emsp;&emsp;&emsp; [danglingポインタ](#SS_4_8_3)  
&emsp;&emsp;&emsp; [Most Vexing Parse](#SS_4_8_4)  

&emsp;&emsp; [ソフトウェア一般](#SS_4_9)  
&emsp;&emsp;&emsp; [ヒープ](#SS_4_9_1)  
&emsp;&emsp;&emsp; [スレッドセーフ](#SS_4_9_2)  
&emsp;&emsp;&emsp; [リエントラント](#SS_4_9_3)  
&emsp;&emsp;&emsp; [クリティカルセクション](#SS_4_9_4)  
&emsp;&emsp;&emsp; [スピンロック](#SS_4_9_5)  
&emsp;&emsp;&emsp; [ハンドル](#SS_4_9_6)  
&emsp;&emsp;&emsp; [フリースタンディング環境](#SS_4_9_7)  
&emsp;&emsp;&emsp; [サイクロマティック複雑度](#SS_4_9_8)  
&emsp;&emsp;&emsp; [凝集性](#SS_4_9_9)  
&emsp;&emsp;&emsp;&emsp; [凝集性の欠如](#SS_4_9_9_1)  
&emsp;&emsp;&emsp;&emsp; [LCOM](#SS_4_9_9_2)  

&emsp;&emsp;&emsp; [Spurious Wakeup](#SS_4_9_10)  
&emsp;&emsp;&emsp; [Static Initialization Order Fiasco(静的初期化順序問題)](#SS_4_9_11)  
&emsp;&emsp;&emsp; [副作用](#SS_4_9_12)  
&emsp;&emsp;&emsp; [Itanium C++ ABI](#SS_4_9_13)  

&emsp;&emsp; [C++コンパイラ](#SS_4_10)  
&emsp;&emsp;&emsp; [g++](#SS_4_10_1)  
&emsp;&emsp;&emsp; [clang++](#SS_4_10_2)  

&emsp;&emsp; [非ソフトウェア用語](#SS_4_11)  
&emsp;&emsp;&emsp; [セマンティクス](#SS_4_11_1)  
&emsp;&emsp;&emsp; [割れ窓理論](#SS_4_11_2)  
&emsp;&emsp;&emsp; [車輪の再発明](#SS_4_11_3)  
...
bird::flyのオーバーライド関数(penguin::fly)について、[リスコフの置換原則(LSP)](#SS_5_3)に反した例を下記する。
...
kyukanchoとstd::stringの関係を[has-a](#SS_4_1_2)の関係と呼ぶ。
...
自身の機能を提供する場合に使われる。
[has-a](#SS_4_1_2)の関係は、is-implemented-in-terms-of の関係の一種である。

is-implemented-in-terms-ofは下記の手段1-3に示した方法がある。

* 手段1.[public継承によるis-implemented-in-terms-of](#SS_4_1_3_1)  
* 手段2.[private継承によるis-implemented-in-terms-of](#SS_4_1_3_2)  
* 手段3.[コンポジションによる(has-a)is-implemented-in-terms-of](#SS_4_1_3_3)  
...
手段1は[is-a](#SS_4_1_1)に見え、以下に示すような問題も考慮する必要があるため、
可読性、保守性を劣化させる可能性がある。
...
すでに述べたようにこの方法は、
[private継承によるis-implemented-in-terms-of](#SS_4_1_3_2)や、
[コンポジションによる(has-a)is-implemented-in-terms-of](#SS_4_1_3_3)
と比べコードがシンプルになる。 
...
この方法は、[public継承によるis-implemented-in-terms-of](#SS_4_1_3_1)が持つデストラクタ問題は発生せす、
[is-a](#SS_4_1_1)と誤解してしまう問題も発生しない。
...
* 下記のようなコードはstd::shared_ptrの仕様が想定する[セマンティクス](#SS_4_11_1)に沿っておらず、
...
X、Yオブジェクトへの[ハンドル](#SS_4_9_6)を完全に失った状態であり、X、Yオブジェクトを解放する手段はない。
...
下記に示す通り、このコードは、
[オープン・クローズドの原則(OCP)](#SS_5_2)にも対応した柔軟な構造を実現している。
...
従って、これらのオブジェクトに対して[等価性のセマンティクス](#SS_4_3_1)
...
下記の単体テストから明らかな通り、
「[等価性のセマンティクス](#SS_4_3_1)」で示した最後の例も、copyセマンティクスを満たしていない。
...
原因は、copy代入で[スライシング](#SS_4_7_3)が起こるためである。
...
4. 効率性  
   ...
   これは、リソースの複製を避けることで達成される(「[moveセマンティクス](#SS_4_3_3)」参照)。
...
3. 正しいセマンティクス  
   copy代入によって代入元のオブジェクトが変更されてはならない(「[copyセマンティクス](#SS_4_3_2)」参照)。
...
関数構造の適・不適については、[サイクロマティック複雑度](#SS_4_9_8)によって下記テーブルのように定義する。
...
リソース管理を専門とするクラス
(例: 標準ライブラリの[RAII(scoped guard)](#SS_6_10)クラス)に任せる設計ガイドラインを指す。
...
自動生成されるメンバ関数では、
複雑なリソース管理の要件を満たせないことがある(「[シャローコピー](#SS_4_7_1)」参照)。
...
* [等価性のセマンティクス](#SS_4_3_1)
* [copyセマンティクス](#SS_4_3_2)
* [moveセマンティクス](#SS_4_3_3)

に従わなけならない。

### クラス凝集性のクライテリア <a id="SS_4_5_3"></a>
クラス構造の適・不適については、[LCOM](#SS_4_9_9_2)によって下記テーブルのように定義する。
...
* `LCOM <= 0.4`  
  クラスが非常に凝集しており、[単一責任の原則(SRP)](#SS_5_1)を強く遵守している状態であるため、
  通常、デザインの見直しは不要である。
...
このスタイルは、C言語由来の「`const`を左に置く」スタイル([west-const](#SS_4_6_3))に比べ、
...
copy代入演算子が行うコピーであり、[ディープコピー](#SS_4_7_2)と対比的に使われる概念である。
...
ディープコピーとは、[シャローコピー](#SS_4_7_1)が発生させる問題を回避したコピーである。
...
スライシングが起こった場合、そうならないことが問題である(「[等価性のセマンティクス](#SS_4_3_1)」参照)。
...
danglingポインタとは、[danglingリファレンス](#SS_4_8_2)と同じような状態になったポインタを指す。
...
一般に、リエントラントな関数は[スレッドセーフ](#SS_4_9_2)であるが、逆は成り立たない。
...
「[凝集性の欠如](#SS_4_9_9_1)(LCOM)」というメトリクスで計測される。

* [凝集性の欠如](#SS_4_9_9_1)メトリクスの値が1に近ければ凝集性は低く、この値が0に近ければ凝集性は高い。
* メンバ変数やメンバ関数が多くなれば、凝集性は低くなりやすい。
* 凝集性は、クラスのメンバがどれだけ一貫した責任を持つかを示す。
* 「[単一責任の原則(SRP)](#SS_5_1)」を守ると凝集性は高くなりやすい。
* 「[Accessor](#SS_6_5)」を多用すれば、振る舞いが分散しがちになるため、通常、凝集性は低くなる。
...
(ただし、「[Immutable](#SS_6_7)」の観点からは、QuadraticEquation::Set()がない方が良い)。
...
[凝集性](#SS_4_9_9)の欠如(Lack of Cohesion in Methods/LCOM)とは、
...
クラスデザイン見直しの基準値としてLCOMを活用する場合、[クラス凝集性のクライテリア](#SS_4_5_3)に具体的な推奨値を示す。

#### LCOM <a id="SS_4_9_9_2"></a>
[凝集性の欠如](#SS_4_9_9_1)とはLack of Cohesion in Methodsの和訳であり、LCOMと呼ばれる。
...
[g++](#SS_4_10_1)や[clang++](#SS_4_10_2)はx86/x86-64やARM64など多くのプラットフォームでもItaniumC++ABI準拠の規約を採用している。
...
* [単一責任の原則(SRP)](#SS_5_1)
* [オープン・クローズドの原則(OCP)](#SS_5_2)
* [リスコフの置換原則(LSP)](#SS_5_3)
* [インターフェース分離の原則(ISP)](#SS_5_4)
* [依存関係逆転の原則(DIP)](#SS_5_5)
...
後述する「[依存関係逆転の原則(DIP)](#SS_5_5)」にも反している。
...
(実際には、TransactorIFから派生する具象クラスの生成用Factory関数(「[Factory](#SS_6_17)」参照)
...
[Pimpl](#SS_6_3)のようにラッピングを使用するパターンも有用である。
...
ところが実際にはオブジェクトの[スライシング](#SS_4_7_3)という別の問題を引き起こす。  
...
&emsp;&emsp; [ガード節](#SS_6_1)  
&emsp;&emsp; [BitmaskType](#SS_6_2)  
&emsp;&emsp; [Pimpl](#SS_6_3)  
&emsp;&emsp; [lightweight Pimpl](#SS_6_4)  
&emsp;&emsp; [Accessor](#SS_6_5)  
&emsp;&emsp; [Copy-And-Swap](#SS_6_6)  
&emsp;&emsp; [Immutable](#SS_6_7)  
&emsp;&emsp; [Clone(仮想コンストラクタ)](#SS_6_8)  
&emsp;&emsp; [NVI(non virtual interface)](#SS_6_9)  
&emsp;&emsp; [RAII(scoped guard)](#SS_6_10)  
&emsp;&emsp; [Future](#SS_6_11)  
&emsp;&emsp; [DI(dependency injection)](#SS_6_12)  
&emsp;&emsp; [Singleton](#SS_6_13)  
&emsp;&emsp; [State](#SS_6_14)  
&emsp;&emsp; [Null Object](#SS_6_15)  
&emsp;&emsp; [Templateメソッド](#SS_6_16)  
&emsp;&emsp; [Factory](#SS_6_17)  
&emsp;&emsp; [Named Constructor](#SS_6_18)  
&emsp;&emsp; [Proxy](#SS_6_19)  
&emsp;&emsp; [Strategy](#SS_6_20)  
&emsp;&emsp; [Visitor](#SS_6_21)  
&emsp;&emsp; [CRTP(curiously recurring template pattern)](#SS_6_22)  
&emsp;&emsp; [Observer](#SS_6_23)  
&emsp;&emsp; [MVC](#SS_6_24)  
&emsp;&emsp; [Cでのクラス表現](#SS_6_25)  
...
ような場合に使用する。
そのため[オープン・クローズドの原則(OCP)](#SS_5_2)の実装方法としても有用である。
...
[Pimpl](#SS_6_3)の解説で示したように依存関係をシンプルに保つには極めて有効なパターンではあるが、
...
一方で、「[Accessor](#SS_6_5)」で紹介したsetterは、クラスのimmutabilityを下げる。
...
オブジェクトコピーによる[スライシング](#SS_4_7_3)を回避するためのイデオムである。
...
(もしくは[Singleton](#SS_6_13)オブジェクトや静的オブジェクト(std::coutやstd::cin等)に直接アクセスする)
クラスNotDIがあるとする。
...
この性質は、[Factory](#SS_6_17)や[Named Constructor](#SS_6_18)の実装には不可欠であるが、
...
DI(「[DI(dependency injection)](#SS_6_12)」参照)と組み合わせて使われることが多い。

「[Templateメソッド](#SS_6_16)」の例にFactoryを適用したソースコードを下記する。
...
Named Connstructorは、[Singleton](#SS_6_13)のようなオブジェクトを複数、生成するためのパターンである。
...
次に示したのは「[Factory](#SS_6_17)」の例にこのパターンを適応したコードである。
...
その属性の追加に対して「[オープン・クローズドの原則(OCP)](#SS_5_2)」に対応した構造となった。
...
あるいは、[DI(dependency injection)](#SS_6_12)と組み合わせて、下記のような改善も有用である。
...
これを改めるためには、「[CRTP(curiously recurring template pattern)](#SS_6_22)」が必要になる。

次に示すソースコードはVisitorとは関係がないが、
FileEntityVisitorから派生するクラスを下記クラス図が示すように改善することで、
単体テストが容易になる例である(「[DI(dependency injection)](#SS_6_12)」参照)。
...
このパターンを用いて、「[Visitor](#SS_6_21)」のFileEntityの3つの派生クラスが持つコードクローン
...
GUIアプリケーションを[MVC](#SS_6_24)で実装する場合のModelがSubjectであり、
ViewがObserverNである。
...
ViewはModelの[Observer](#SS_6_23)であるため、ModelはViewへ依存しない。
