...
* &&や||の論理演算子の右オペランドで[副作用](term_explanation.md#SS_19_20_6)のある処理をしない。
...
* 上記例を除き、sizeof演算子のオペランドは一見[副作用](term_explanation.md#SS_19_20_6)を持っているような式を含んではならない。
...
* assert()はコンパイルオプションにより無効化されることがあるため、
  assert()の引数に[副作用](term_explanation.md#SS_19_20_6)のある式を入れない。
