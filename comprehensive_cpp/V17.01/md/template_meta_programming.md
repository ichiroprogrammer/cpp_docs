...
# 13 テンプレートメタプログラミング <a id="SS_13"></a>
...
## 13.1 ログ取得ライブラリの開発 <a id="SS_13_1"></a>
ここではログ取得ライブラリの開発を行う。

### 13.1.1 要件 <a id="SS_13_1_1"></a>
...
### 13.1.2 ログ取得ライブラリのインターフェース <a id="SS_13_1_2"></a>
...
### 13.1.3 パラメータパック <a id="SS_13_1_3"></a>
...
* 可変長引数を持つ
* [算術型](term_explanation.md#SS_18_1_1)の引数と戻り値を持つ
* すべての引数の和を返す
...
[演習-パラメータパック](exercise_q.md#SS_19_11_1)  

#### 13.1.3.1 パラメータパックの畳みこみ式 <a id="SS_13_1_3_1"></a>
...
#### 13.1.3.2 前から演算するパラメータパック <a id="SS_13_1_3_2"></a>
...
(「[浮動小数点型](programming_convention.md#SS_3_1_1_5)」参照)
...
### 13.1.4 Loggerの実装 <a id="SS_13_1_4"></a>
...
### 13.1.5 ユーザ定義型とそのoperator\<\<のname lookup <a id="SS_13_1_5"></a>

ここで、一旦Logging::Loggerの開発を止め、
Logging::Logger::set_innerでのApp::operator<<の[name lookup](term_explanation.md#SS_18_4_2)について考えてみることにする。
...
その認識とは、「テンプレート内の識別子の[name lookup](term_explanation.md#SS_18_4_2)は、
それがインスタンス化される時に行われる」というものであり、これにより
「 Logging::Loggerのname lookupは単体テスト内で行われる。
それはApp::operator<<宣言後であるためコンパイルできる」と考えることができるが、
[two phase name lookup](term_explanation.md#SS_18_4_3)で行われるプロセスと反するため誤りである。
...
[ADL](term_explanation.md#SS_18_4_5)(実引数依存探索)が働く。
また、Logging::Logger::set_inner(x)はテンプレートであるため、[two phase name lookup](term_explanation.md#SS_18_4_3)
が使用される。
...
### 13.1.6 Ints_tのログ登録 <a id="SS_13_1_6"></a>
話題はログ取得ライブラリの開発に戻る。
...
instsの[関連名前空間](term_explanation.md#SS_18_4_6)もstdであり、Appではない。
その結果App::operator<<は発見できず、このようなエラーになった。

LOGGERからApp::operator<<を使う場合の単体テストは下記のようになるが、
[ADL](term_explanation.md#SS_18_4_5)によってLogging::Logger::set_inner(ints)内に導入される名前空間はstdのみであり、
前記単体テスト同様にコンパイルできない。
...
#### 13.1.6.1 operator\<\<をstd内で宣言する <a id="SS_13_1_6_1"></a>
ここで解決したい問題は、すでに示した通り、
「[ADL](term_explanation.md#SS_18_4_5)によってLogging::Logger::set_inner(ints)内に導入される名前空間はstdである」
...
上記コードは[two phase name lookup](term_explanation.md#SS_18_4_3)等の効果により、想定通りコンパイルできるが、
...
#### 13.1.6.2 operator\<\<をグローバル名前空間内で宣言する <a id="SS_13_1_6_2"></a>
すでに述べた通り、
...
このドキュメントで使用している[g++](term_explanation.md#SS_18_9_1)ではこのコードはコンパイルでき、
動作も問題ないように思われるが、[clang++](term_explanation.md#SS_18_9_2)では以下のようなエラーが発生し、コンパイルできない。
...
この理由は「[two phase name lookup](term_explanation.md#SS_18_4_3)」の後半で詳しく解説したので、ここでは繰り返さないが、
...
* operator<<(std::ostream& os, App::Ints_t const& ints)
  ...
  (「[割れ窓理論](term_explanation.md#SS_18_12_1)」参照)になってしまうかもしれない)。
...
#### 13.1.6.3 operator\<\<をLogging内で宣言する <a id="SS_13_1_6_3"></a>
...
#### 13.1.6.4 Logging::Logger::set_inner(ints)内でusing namespace Appを行う <a id="SS_13_1_6_4"></a>
...
#### 13.1.6.5 Ints_tを構造体としてApp内に宣言する <a id="SS_13_1_6_5"></a>

App::Ints_t用のoperator<<がLogging::Logger::set_inner内でname lookup出来ない理由は、
これまで述べてきたようにApp::Inst_tの[関連名前空間](term_explanation.md#SS_18_4_6)がAppではなく、stdになってしまうからである。

これを回避するためにはその原因を取り払えばよく、
つまり、App::Inst_tの[関連名前空間](term_explanation.md#SS_18_4_6)がAppになるようにすればよい。
...
* using宣言によりstd::vectorのすべてのコンストラクタをApp::Ints_tに導入
  (「[継承コンストラクタ](term_explanation.md#SS_18_2_3)」参照)

としているため、エイリアスで宣言されたInts_tと等価である。
C++03では、[継承コンストラクタ](term_explanation.md#SS_18_2_3)が使えなかったため、
上記のような構造体を定義するためには、
...
#### 13.1.6.6 operator\<\<を使わない <a id="SS_13_1_6_6"></a>
...
#### 13.1.6.7 Ints_tのログ登録のまとめ <a id="SS_13_1_6_7"></a>
...
[演習-名前空間による修飾不要なoperator\<\<](exercise_q.md#SS_19_11_3)  

## 13.2 Nstdライブラリの開発 <a id="SS_13_2"></a>

「[operator\<\<を使わない](template_meta_programming.md#SS_13_1_6_6)」で導入したコードは、短いながらも汎用性が高い。
このようなコードをローカルなファイルに閉じ込めてしまうと、
コードクローンや、[車輪の再発明](term_explanation.md#SS_18_12_2)による開発効率の低下につながることがある。
...
### 13.2.1 Nstdライブラリを使用したリファクタリング <a id="SS_13_2_1"></a>
...
### 13.2.2 安全なvector <a id="SS_13_2_2"></a>
...
[演習-std::arrayの継承](exercise_q.md#SS_19_11_4)  

### 13.2.3 安全な配列型コンテナ <a id="SS_13_2_3"></a>
...
これを初期化するためには初期化子リスト
(「[初期化子リストコンストラクタ](term_explanation.md#SS_18_2_2)」、
「[一様初期化](term_explanation.md#SS_18_2_7)」参照)を用いるのが良い。
...
### 13.2.4 初期化子リストの副作用 <a id="SS_13_2_4"></a>
...
[演習-エイリアステンプレート](exercise_q.md#SS_19_11_2)  

### 13.3.1 STLのtype_traits <a id="SS_13_3_1"></a>
...
#### 13.3.1.1 std::true_type/std::false_type <a id="SS_13_3_1_1"></a>
...
#### 13.3.1.2 std::integral_constant <a id="SS_13_3_1_2"></a>
...
#### 13.3.1.3 std::is_same <a id="SS_13_3_1_3"></a>
...
#### 13.3.1.4 std::enable_if <a id="SS_13_3_1_4"></a>
...
* std::enable_if\<true>::typeは[well-formed](term_explanation.md#SS_18_10_9)
* std::enable_if\<false>::typeは[ill-formed](term_explanation.md#SS_18_10_8)
...
std::enable_ifのこの特性と後述する[SFINAE](term_explanation.md#SS_18_4_7)により、
様々な静的ディスパッチを行うことができる。


#### 13.3.1.5 std::conditional <a id="SS_13_3_1_5"></a>
...
#### 13.3.1.6 std::is_void <a id="SS_13_3_1_6"></a>
...
### 13.3.2 is_void_xxxの実装 <a id="SS_13_3_2"></a>
...
#### 13.3.2.1 is_void_fの実装 <a id="SS_13_3_2_1"></a>
...
[演習-SFINAEを利用しない関数テンプレートの特殊化によるis_void](exercise_q.md#SS_19_11_5)

#### 13.3.2.2 is_void_sの実装 <a id="SS_13_3_2_2"></a>
...
[演習-SFINAEを利用しないクラステンプレートの特殊化によるis_void](exercise_q.md#SS_19_11_6)

#### 13.3.2.3 is_void_sfinae_fの実装 <a id="SS_13_3_2_3"></a>
[SFINAE](term_explanation.md#SS_18_4_7)を使用した関数テンプレートis_void_sfinae_fの実装は以下のようになる。
...
であるため、Tがvoidの時のみ[name lookup](term_explanation.md#SS_18_4_2)の対象になる。
...
であるため、Tが非voidの時のみ[name lookup](term_explanation.md#SS_18_4_2)の対象になる。
...
[演習-SFINAEを利用した関数テンプレートの特殊化によるis_void](exercise_q.md#SS_19_11_7)

#### 13.3.2.4 is_void_sfinae_sの実装 <a id="SS_13_3_2_4"></a>
[SFINAE](term_explanation.md#SS_18_4_7)を使用したクラステンプレートis_void_sfinae_sの実装は以下のようになる。
...
is_void_sfinae_sの特殊化が[name lookup](term_explanation.md#SS_18_4_2)の対象の中に見つからなかった場合、
これが使われる。
...
「[is_void_ena_sの実装](template_meta_programming.md#SS_13_3_2_5)」でその例を示す。

[演習-SFINAEを利用したクラステンプレートの特殊化によるis_void](exercise_q.md#SS_19_11_8)

#### 13.3.2.5 is_void_ena_sの実装 <a id="SS_13_3_2_5"></a>
[std::enable_if](template_meta_programming.md#SS_13_3_1_4)による[SFINAE](term_explanation.md#SS_18_4_7)とクラステンプレートの特殊化を使用した
...
この例では、「[is_void_sfinae_sの実装](template_meta_programming.md#SS_13_3_2_4)」の
...
で行っている。
std::enable_ifの値パラメータis_void_f\<T>()は、「[is_void_fの実装](template_meta_programming.md#SS_13_3_2_1)」で示したものである。

単体テストは、「[is_void_sfinae_sの実装](template_meta_programming.md#SS_13_3_2_4)」で示したものとほぼ同様で、以下のようになる。
...
#### 13.3.2.6 is_void_cond_sの実装 <a id="SS_13_3_2_6"></a>
...
std::conditionalの値パラメータis_void_f\<T>()は、「[is_void_fの実装](template_meta_programming.md#SS_13_3_2_1)」で示したものである。
この例では、SFINAEもクラステンプレートの特殊化も使用していないが、
下記単体テストからわかる通り、「[is_void_sfinae_sの実装](template_meta_programming.md#SS_13_3_2_4)」と同じ機能を備えている。
...
### 13.3.3 is_same_xxxの実装 <a id="SS_13_3_3"></a>
...
#### 13.3.3.1 is_same_fの実装 <a id="SS_13_3_3_1"></a>
...
#### 13.3.3.2 is_same_vの実装 <a id="SS_13_3_3_2"></a>
...
#### 13.3.3.3 is_same_sの実装 <a id="SS_13_3_3_3"></a>
...
「[is_same_vの実装](template_meta_programming.md#SS_13_3_3_2)」と同様に単純であるため、解説は不要だろう。 単体テストは以下のようになる。
...
#### 13.3.3.4 is_same_sfinae_fの実装 <a id="SS_13_3_3_4"></a>
...
#### 13.3.3.5 is_same_sfinae_sの実装 <a id="SS_13_3_3_5"></a>
...
「[is_void_sfinae_sの実装](template_meta_programming.md#SS_13_3_2_4)」とほぼ同様であるため、解説は不要だろう。 
単体テストは以下のようになる。
...
#### 13.3.3.6 is_same_templの実装 <a id="SS_13_3_3_6"></a>
...
[演習-テンプレートテンプレートパラメータ](exercise_q.md#SS_19_11_9)

#### 13.3.3.7 IsSameSomeOfの実装 <a id="SS_13_3_3_7"></a>
...
[演習-テンプレートパラメータを可変長にしたstd::is_same](exercise_q.md#SS_19_11_10)

### 13.3.4 AreConvertibleXxxの実装 <a id="SS_13_3_4"></a>
...
#### 13.3.4.1 AreConvertibleの実装 <a id="SS_13_3_4_1"></a>
...
「[IsSameSomeOfの実装](template_meta_programming.md#SS_13_3_3_7)」のコードパターンとほぼ同様であるため、解説は不要だろうが、
...
#### 13.3.4.2 AreConvertibleWithoutNarrowConvの実装 <a id="SS_13_3_4_2"></a>
...
上記を抜粋した下記のコードは「縮小型変換を発生さる{}による初期化は[ill-formed](term_explanation.md#SS_18_10_8)になる」
ことをSFINAEに利用している。
...
### 13.3.5 関数の存在の診断 <a id="SS_13_3_5"></a>
...
#### 13.3.5.1 exists_void_func_sfinae_fの実装 <a id="SS_13_3_5_1"></a>
...
#### 13.3.5.2 exists_void_func_sfinae_sの実装 <a id="SS_13_3_5_2"></a>
...
#### 13.3.5.3 exists_void_func_sfinae_s2の実装 <a id="SS_13_3_5_3"></a>
...
[演習-メンバ関数の存在の診断](exercise_q.md#SS_19_11_11)

#### 13.3.5.4 exists_begin/exsits_endの実装 <a id="SS_13_3_5_4"></a>
「テンプレートパラメータTに対して、std::begin(T)が存在するか否かの診断」をするexists_beginの実装は、
「[exists_void_func_sfinae_sの実装](template_meta_programming.md#SS_13_3_5_2)」
...
* [ill-formed](term_explanation.md#SS_18_10_8)ならばill-formedになる
* [well-formed](term_explanation.md#SS_18_10_9)ならvoidを生成する
...
の戻り型が配列型の[rvalue](term_explanation.md#SS_18_5_3)である"int (&&) [3]"となり、
...
そのオブジェクトが[lvalue](term_explanation.md#SS_18_5_2)(この例ではint (&)[3])であれば、
...
#### 13.3.5.5 IsRangeの実装 <a id="SS_13_3_5_5"></a>
...
[演習-範囲for文のオペランドになれるかどうかの診断](exercise_q.md#SS_19_11_12)

#### 13.3.5.6 exists_put_to_as_memberの実装 <a id="SS_13_3_5_6"></a>
...
「[exists_void_func_sfinae_fの実装](template_meta_programming.md#SS_13_3_5_1)」と同様のパターンを使用したので解説は不要だろう。
...
#### 13.3.5.7 exists_put_to_as_non_memberの実装 <a id="SS_13_3_5_7"></a>
...
「[exists_begin/exsits_endの実装](template_meta_programming.md#SS_13_3_5_4)」と「[exists_put_to_as_memberの実装](template_meta_programming.md#SS_13_3_5_6)」
で使用したパターンを混合しただけなので解説や単体テストは省略する。


#### 13.3.5.8 ExistsPutToの実装 <a id="SS_13_3_5_8"></a>
...
「[IsRangeの実装](template_meta_programming.md#SS_13_3_5_5)」に影響されて、一旦このように実装したが、先に書いた通り、
...
#### 13.3.5.9 ValueTypeの実装 <a id="SS_13_3_5_9"></a>
下記で示す通り、
...
[演習-配列の長さの取り出し](exercise_q.md#SS_19_11_13)
[演習-配列の次元の取り出し](exercise_q.md#SS_19_11_14)

## 13.4 Nstdライブラリの開発2 <a id="SS_13_4"></a>
ここでは予定していた通りSafeArray2を開発し、その後Nstdに必要なライブラリの開発を続ける。


### 13.4.1 SafeArray2の開発 <a id="SS_13_4_1"></a>
...
### 13.4.2 Nstd::SafeIndexの開発 <a id="SS_13_4_2"></a>
...
(「[is_same_templの実装](template_meta_programming.md#SS_13_3_3_6)」参照)を使用し下記のように書ける。
...
### 13.4.3 Nstd::SafeIndexのoperator\<\<の開発 <a id="SS_13_4_3"></a>
...
「[メタ関数のテクニック](template_meta_programming.md#SS_13_3)」で紹介したSFINAEにより、この問題は下記のように対処できる。
...
### 13.4.4 コンテナ用Nstd::operator\<\<の開発 <a id="SS_13_4_4"></a>
...
実際、現在使用中の[g++](term_explanation.md#SS_18_9_1)ではこのコードはコンパイルできず、
...
## 13.5 ログ取得ライブラリの開発2 <a id="SS_13_5"></a>
...
## 13.6 その他のテンプレートテクニック <a id="SS_13_6"></a>
ここでは、これまでの議論の対象にならなかったテンプレートのテクニックや注意点について記述する。

### 13.6.1 ユニバーサルリファレンスとstd::forward <a id="SS_13_6_1"></a>
...
[ユニバーサルリファレンス](term_explanation.md#SS_18_6_1)を使って下記のように書き直した。
...
この原因は、
「関数が受け取った[rvalue](term_explanation.md#SS_18_5_3)リファレンスは、
その関数から別の関数に受け渡される時に[lvalue](term_explanation.md#SS_18_5_2)リファレンスとして扱われる」からである。
...
(T const&&はconstな[rvalue](term_explanation.md#SS_18_5_3)リファレンスである)、
ユニバーサルリファレンスが[lvalue](term_explanation.md#SS_18_5_2)リファレンスであった場合は、
...
なお、ユニバーサルリファレンスは、[リファレンスcollapsing](term_explanation.md#SS_18_6_4)の一機能としても理解できる。

### 13.6.2 ジェネリックラムダ <a id="SS_13_6_2"></a>
...
#### 13.6.2.1 ジェネリックラムダの内部構造 <a id="SS_13_6_2_1"></a>
...
#### 13.6.2.2 std::variantとジェネリックラムダ <a id="SS_13_6_2_2"></a>
...
ここで示した関数テンプレートは、デザインパターン[Visitor](design_pattern.md#SS_9_20)の例であり、
...
### 13.6.3 クラステンプレートと継承の再帰構造 <a id="SS_13_6_3"></a>
クラステンプレートと継承の再帰構造はCRTPと呼ばれる。
このコードパターンについては、
「[CRTP(curiously recurring template pattern)](design_pattern.md#SS_9_21)」で説明している。

### 13.6.4 constexpr if文 <a id="SS_13_6_4"></a>
...
このような場合、[SFINAE](term_explanation.md#SS_18_4_7)によるオーバーロードが必須であったが、
...
### 13.6.5 意図しないname lookupの防止 <a id="SS_13_6_5"></a>
...
[ADL](term_explanation.md#SS_18_4_5)(実引数依存探索)により、Appもis_equalの[name lookup](term_explanation.md#SS_18_4_2)の対象になるからである。
...
* [ジェネリックすぎるテンプレートを書かない](template_meta_programming.md#SS_13_6_5_1)
* [ADLが本当に必要でない限り名前を修飾する](template_meta_programming.md#SS_13_6_5_2)
* [ADL Firewallを使う](template_meta_programming.md#SS_13_6_5_3)

のようにいくつか考えられる。これらについて以下で説明を行う。

#### 13.6.5.1 ジェネリックすぎるテンプレートを書かない <a id="SS_13_6_5_1"></a>
...
[ジェネリックラムダ](template_meta_programming.md#SS_13_6_2)を使えばよい。
...
#### 13.6.5.2 ADLが本当に必要でない限り名前を修飾する <a id="SS_13_6_5_2"></a>
下記のコードについて考える。
...
([two phase name lookup](term_explanation.md#SS_18_4_3)の一回目のname lookupでfがバインドされるため)。
...
[ADL](term_explanation.md#SS_18_4_5)や[two phase name lookup](term_explanation.md#SS_18_4_3)が絡む場合ならなおさらである。

こういう意図しない[name lookup](term_explanation.md#SS_18_4_2)に備えるためには、
...
#### 13.6.5.3 ADL Firewallを使う <a id="SS_13_6_5_3"></a>
下記のコードについて考える。
...
XY型オブジェクトを引数にした関数呼び出しによる[関連名前空間](term_explanation.md#SS_18_4_6)は、
...
### 13.6.6 Nstd::Type2Strの開発 <a id="SS_13_6_6"></a>
...
等によるコンパイルエラーとの戦いである。
また、これをクリアしても[two phase name lookup](term_explanation.md#SS_18_4_3)や[ADL](term_explanation.md#SS_18_4_5)が次の関門になる。
...
残念なことにこの関数は非標準であるため、
それを使っているNstd::Inner\_::demangleは[g++](term_explanation.md#SS_18_9_1)/[clang++](term_explanation.md#SS_18_9_2)
でなければコンパイルできないだろう。
...
### 13.6.7 静的な文字列オブジェクト <a id="SS_13_6_7"></a>
...
#### 13.6.7.1 StaticStringのヘルパークラスの開発 <a id="SS_13_6_7_1"></a>
...
#### 13.6.7.2 StaticStringの開発 <a id="SS_13_6_7_2"></a>
...
#### 13.6.7.3 整数をStaticStringに変換する関数の開発 <a id="SS_13_6_7_3"></a>
...
#### 13.6.7.4 ファイル位置を静的に保持したエクセプションクラスの開発 <a id="SS_13_6_7_4"></a>
「[静的な文字列オブジェクト](template_meta_programming.md#SS_13_6_7)」で見たように、
...
### 13.6.8 関数型をテンプレートパラメータで使う <a id="SS_13_6_8"></a>
ここで使う「関数型」とは、
...
「[RAII(scoped guard)](design_pattern.md#SS_9_9)でも示したScopedGuardの実装を下記する。
...
[演習-関数型のテンプレートパラメータを持つクラステンプレート](exercise_q.md#SS_19_11_15)  

## 13.7 注意点まとめ <a id="SS_13_7"></a>
本章では、テンプレートメタプログラミングのテクニックや注意点について解説したが、
本章の情報量は多く、また他の章で行ったものもあるため以下にそれらをまとめる。

* [name lookup](term_explanation.md#SS_18_4_2)には複雑なルールが適用されるため、非直感的なバインドが行われる場合がある。
  従って、テンプレートライブラリの開発には単体テストは必須である。

* 使用しているコンパイラが[two phase name lookup](term_explanation.md#SS_18_4_3)をサポートしているか否かに気を付ける。
...
* [ADL](term_explanation.md#SS_18_4_5)を利用しない場合、テンプレートで使う識別子は名前空間名やthis->等で修飾する
  (「[意図しないname lookupの防止](template_meta_programming.md#SS_13_6_5)」参照)。
...
* テンプレートのインターフェースではないが、実装の都合上ヘッダファイルに記述する定義は、
  ...
  外部から参照しない(「[is_void_sfinae_fの実装](template_meta_programming.md#SS_13_3_2_3)」参照)。
...
* ユニバーサルリファレンス引数を他の関数に渡すのであれば、std::forwardを使う
  (「[ユニバーサルリファレンス](term_explanation.md#SS_18_6_1)」、「[ユニバーサルリファレンスとstd::forward](template_meta_programming.md#SS_13_6_1)」参照)。

* 関数テンプレートとその特殊化はソースコード上なるべく近い位置で定義する
  (「[two phase name lookup](term_explanation.md#SS_18_4_3)」参照)。

* [two phase name lookup](term_explanation.md#SS_18_4_3)により意図しない副作用が発生する可能性があるため、
...
* 関数テンプレートのオーバーロードと特殊化の[name lookup](term_explanation.md#SS_18_4_2)の優先度に気を付ける。
...
* ユニバーサルリファレンスを持つ関数テンプレートをオーバーロードしない。
  「[ユニバーサルリファレンスとstd::forward](template_meta_programming.md#SS_13_6_1)」で述べたように、
...
* テンプレートに関数型オブジェクトを渡す場合、リファレンスの付け忘れに気を付ける
  (「[関数型をテンプレートパラメータで使う](template_meta_programming.md#SS_13_6_8)」
  、「[現象6 - STLのバグ?](debug.md#SS_15_9_6)」
  参照)。

* 意図しないテンプレートパラメータによるインスタンス化の防止や、
  コンパイルエラーを解読しやすくするために、
  適切にstatic_assert(「[exists_begin/exsits_endの実装](template_meta_programming.md#SS_13_3_5_4)」
  、「[assertion](programming_convention.md#SS_3_11_1)」
  参照)を使う。
...
* ランタイム時の処理を削減する、static_assertを適切に用いる等の目的のために、
  ...
  、「[constexpr関数](programming_convention.md#SS_3_3_3_8)」
  参照)。
