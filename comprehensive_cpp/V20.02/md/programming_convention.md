...
* 小数演算に安易に[浮動小数点型](term_explanation.md#SS_19_1_9)を使用せず、
  [浮動小数点型のダイナミックレンジ](term_explanation.md#SS_19_1_9_1)が必要な場合のみにこれらの型を使用する。
  ちなみに銀河系の直径は1e+21メートル程度、プランク長は1.616229e-35メートルであるため、
  銀河から素粒子までのサイズを一つの基本型で表す場合においても、
  floatのダイナミックレンジに収まる。
  従って、floatやdoubleが必要になる場合は極めて限られる。

* [浮動小数点の誤差](term_explanation.md#SS_19_1_9_2)は避けられないため、浮動小数点型のインスタンスを==、!=で比較しない。
...
* 一つの式にfloatとdoubleを混在してしまうと、
  [イプシロン](term_explanation.md#SS_19_1_9_3)を使用した比較の定義ができなくなるので、
...
* INFや、NANを演算で使用しない(「[浮動小数点の演算エラー](term_explanation.md#SS_19_1_9_4)」参照)。
* [汎整数型](term_explanation.md#SS_19_1_4)の演算とは違い、0除算等の[浮動小数点の演算エラー](term_explanation.md#SS_19_1_9_4)は、
...
* インスタンス、インスタンスへのポインタ、インスタンスへのリファレンス等に対して、
    ...
    * constexprにはできないが、const(「[constインスタンス](term_explanation.md#SS_19_3_13)」にはできるインスタンスは、
...
* 単なるデータホルダー(アプリケーションの設定データを保持するようなクラス等)や、
  ...
  [凝集度](term_explanation.md#SS_19_20_2)が高くなるように設計する。
...
* 派生は最大2回程度までに留める。やむを得ず階層が深くなる場合、
  コードの静的解析等を使用し派生関係を明確にする(「[ポリモーフィックなクラス](term_explanation.md#SS_19_3_8)」参照)。
...
* オブジェクトの初期化が完了するまでは派生クラスの仮想関数呼び出し等の
  [RTTI](programming_convention.md#SS_3_5_9)機能を使うことはできないため(「[ポリモーフィックなクラス](term_explanation.md#SS_19_3_8)」参照)、
...
* [サイクロマティック複雑度](term_explanation.md#SS_19_20_3)は15以下が好ましい。
...
* [オーバーライド](programming_convention.md#SS_3_3_2_7)を除き、基底クラスのメンバ関数と同じ名前を持つメンバ関数を派生クラスで宣言、
  定義しない。
  これに反すると[name-hiding](term_explanation.md#SS_19_10_8)のため、基底クラスのメンバ関数の可視範囲を縮小させてしまう。
...
* throwをせざるを得ない場合、最低でも[基本保証](term_explanation.md#SS_19_16_3)をする。
...
* [Run-time Type Information](term_explanation.md#SS_19_3_11)を使用したラインタイム時の型による場合分けは、
...
* [name-hiding](term_explanation.md#SS_19_10_8)を起こすとコードの可読性が著しく低下するため、
...
* 識別子のインポートのための[using宣言](term_explanation.md#SS_19_10_13)は下記のような場合のみに使用する
  (「[継承コンストラクタ](term_explanation.md#SS_19_5_4)」、「[オーバーライドとオーバーロードの違い](term_explanation.md#SS_19_3_9)」参照)。
...
* 下記のような場合を除き、[usingディレクティブ](term_explanation.md#SS_19_10_14)は使用しない。
