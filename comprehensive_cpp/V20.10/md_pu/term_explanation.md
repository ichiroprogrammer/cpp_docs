...
### ユーティリティ <a id="SS_19_9_1"></a>
#### std::move <a id="SS_19_9_1_1"></a>
std::moveは引数を[rvalueリファレンス](term_explanation.md#SS_19_15_2)に変換する関数テンプレートである。

|引数                 |std::moveの動作                                    |
|---------------------|---------------------------------------------------|
|非const [lvalue](term_explanation.md#SS_19_14_1_1)|引数を[rvalueリファレンス](term_explanation.md#SS_19_15_2)にキャストする      |
|const [lvalue](term_explanation.md#SS_19_14_1_1)  |引数をconst [rvalueリファレンス](term_explanation.md#SS_19_15_2)にキャストする|
...
一般に条件変数には、[Spurious Wakeup](term_explanation.md#SS_19_21_5)という問題があり、std::condition_variableも同様である。

上記の抜粋である下記のコード例では[Spurious Wakeup](term_explanation.md#SS_19_21_5)の対策が行われていないため、
意図通り動作しない可能性がある。
...
```deep/plant_uml/mutex_deadlock.pu
@startuml
!theme plain
title デッドロック発生のシナリオ
...
ref_refの型は`int &&`であるが、ref_refの値カテゴリは[rvalue](term_explanation.md#SS_19_14_1_2)ではなく、[lvalue](term_explanation.md#SS_19_14_1_1)である。
...
* リソース管理   
    必須ではないが、aがポインタ等のリソースを保有している場合、
     move代入後にはそのリソースはcに移動していることが一般的である(「[rvalue](term_explanation.md#SS_19_14_1_2)」参照)
...
3. 正しいセマンティクス  
   copy代入によって代入元のオブジェクトが変更されてはならない(「[copyセマンティクス](term_explanation.md#SS_19_18_2)」参照)。
