...
c++20から導入された[コンセプト](core_lang_spec.md#SS_6_11_3)によりジェネリックプログラミングは、
開発容易性、可読性、保守性が大きく向上しため、この章のコード例には、
[コンセプト](core_lang_spec.md#SS_6_11_3)(`concept`, `requires`)を多用した。
...
これは避けるべきなので、「[Singleton](design_pattern.md#SS_3_13)」で述べた構造を導入すると、
...
[パラメータパック](core_lang_spec.md#SS_6_11_4)を使って実装するべきである。

### パラメータパックを使用したテクニック <a id="SS_4_1_3"></a>
[パラメータパック](core_lang_spec.md#SS_6_11_4)を使用するには独特なテクニックが必要となるため、まずは単純な例から説明する。
...
* 可変長引数を持つ
* [算術型](core_lang_spec.md#SS_6_1_3)の引数と戻り値を持つ
* すべての引数の和を返す
...
(「[浮動小数点型](core_lang_spec.md#SS_6_1_12)」参照)
...
Logging::Logger::set_innerでのApp::operator<<の[name lookup](core_lang_spec.md#SS_6_12_2)について考えてみることにする。
...
その認識とは、「テンプレート内の識別子の[name lookup](core_lang_spec.md#SS_6_12_2)は、
それがインスタンス化される時に行われる」というものであり、これにより
「 Logging::Loggerのname lookupは単体テスト内で行われる。
それはApp::operator<<宣言後であるためコンパイルできる」と考えることができるが、
[two phase name lookup](core_lang_spec.md#SS_6_12_3)で行われるプロセスと反するため誤りである。
...
[ADL](core_lang_spec.md#SS_6_12_5)(実引数依存探索)が働く。
また、Logging::Logger::set_inner(x)はテンプレートであるため、[two phase name lookup](core_lang_spec.md#SS_6_12_3)
が使用される。
...
instsの[関連名前空間](core_lang_spec.md#SS_6_12_6)もstdであり、Appではない。
その結果App::operator<<は発見できず、このようなエラーになった。

LOGGERからApp::operator<<を使う場合の単体テストは下記のようになるが、
[ADL](core_lang_spec.md#SS_6_12_5)によってLogging::Logger::set_inner(ints)内に導入される名前空間はstdのみであり、
前記単体テスト同様にコンパイルできない。
...
「[ADL](core_lang_spec.md#SS_6_12_5)によってLogging::Logger::set_inner(ints)内に導入される名前空間はstdである」
...
上記コードは[two phase name lookup](core_lang_spec.md#SS_6_12_3)等の効果により、想定通りコンパイルできるが、
...
このドキュメントで使用している[g++](cpp_idioms.md#SS_8_10_1)ではこのコードはコンパイルでき、
動作も問題ないように思われるが、[clang++](cpp_idioms.md#SS_8_10_2)では以下のようなエラーが発生し、コンパイルできない。
...
この理由は「[two phase name lookup](core_lang_spec.md#SS_6_12_3)」の後半で詳しく解説したので、ここでは繰り返さないが、
...
* operator<<(std::ostream& os, App::Ints_t const& ints)
  ...
  (「[割れ窓理論](cpp_idioms.md#SS_8_11_2)」参照)になってしまうかもしれない)。
...
これまで述べてきたようにApp::Inst_tの[関連名前空間](core_lang_spec.md#SS_6_12_6)がAppではなく、stdになってしまうからである。

これを回避するためにはその原因を取り払えばよく、
つまり、App::Inst_tの[関連名前空間](core_lang_spec.md#SS_6_12_6)がAppになるようにすればよい。
...
* using宣言によりstd::vectorのすべてのコンストラクタをApp::Ints_tに導入
  (「[継承コンストラクタ](core_lang_spec.md#SS_6_6_1_2)」参照)

としているため、エイリアスで宣言されたInts_tと等価である。
C++03では、[継承コンストラクタ](core_lang_spec.md#SS_6_6_1_2)が使えなかったため、
上記のような構造体を定義するためには、
...
コードクローンや、[車輪の再発明](cpp_idioms.md#SS_8_11_3)による開発効率の低下につながることがある。
...
これを初期化するためには初期化子リスト
(「[初期化子リストコンストラクタ](core_lang_spec.md#SS_6_6_1_1)」、
「[一様初期化](core_lang_spec.md#SS_6_6_6)」参照)を用いるのが良い。
...
* [SFINAE](core_lang_spec.md#SS_6_11_1)
...
|is_void_xxx                  |実装方法                                               |
|-----------------------------|-------------------------------------------------------|
|[is_void_f](template_meta_programming.md#SS_4_3_1_1)             |関数テンプレートの特殊化                               |
|[is_void_s](template_meta_programming.md#SS_4_3_1_2)             |クラステンプレートの特殊化                             |
|[is_void_sfinae_f](template_meta_programming.md#SS_4_3_1_3)      |FINAEと関数テンプレートのオーバーロード                |
|[is_void_sfinae_s](template_meta_programming.md#SS_4_3_1_4)      |FINAEとクラステンプレートの特殊化                      |
|[is_void_concept_s](template_meta_programming.md#SS_4_3_1_5)     |コンセプトとクラステンプレートの特殊化                 |
|[is_void_ena_s](template_meta_programming.md#SS_4_3_1_6)         |std::enable_ifによるSFINAEとクラステンプレートの特殊化 |
|[is_void_cond_s](template_meta_programming.md#SS_4_3_1_7)        |std::conditionalと関数テンプレートの特殊化             |
...
[SFINAE](core_lang_spec.md#SS_6_11_1)を使用した関数テンプレートis_void_sfinae_fの実装は以下のようになる。
...
であるため、Tがvoidの時のみ[name lookup](core_lang_spec.md#SS_6_12_2)の対象になる。
...
であるため、Tが非voidの時のみ[name lookup](core_lang_spec.md#SS_6_12_2)の対象になる。
...
[SFINAE](core_lang_spec.md#SS_6_11_1)を使用したクラステンプレートis_void_sfinae_sの実装は以下のようになる。
...
is_void_sfinae_sの特殊化が[name lookup](core_lang_spec.md#SS_6_12_2)の対象の中に見つからなかった場合、
これが使われる。
...
「[is_void_ena_s](template_meta_programming.md#SS_4_3_1_6)の実装」でその例を示す。


#### is_void_concept_s <a id="SS_4_3_1_5"></a>
[is_void_sfinae_s](template_meta_programming.md#SS_4_3_1_4)の実装で使用したSFINAEを回避し、
コンセプトを使用することで可読性の向上が見込める。
以下の実装で使用した[same_as](template_meta_programming.md#SS_4_3_2_6)は\<concepts>で定義されているコンセプトと同様のものである。
...
以下に示した通り、[is_void_sfinae_s](template_meta_programming.md#SS_4_3_1_4)の実装で示した定数テンプレートのテストと同様になっている。
...
この例では、「[is_void_sfinae_s](template_meta_programming.md#SS_4_3_1_4)の実装」の
...
で行っている。
std::enable_ifの値パラメータis_void_f\<T>()は、「[is_void_f](template_meta_programming.md#SS_4_3_1_1)の実装」で示したものである。

単体テストは、「[is_void_sfinae_s](template_meta_programming.md#SS_4_3_1_4)の実装」で示したものとほぼ同様で、以下のようになる。
...
std::conditionalの値パラメータis_void_f\<T>()は、「[is_void_f](template_meta_programming.md#SS_4_3_1_1)の実装」で示したものである。
この例では、[SFINAE](core_lang_spec.md#SS_6_11_1)もクラステンプレートの特殊化も使用していないが、
下記単体テストからわかる通り、「[is_void_sfinae_s](template_meta_programming.md#SS_4_3_1_4)の実装」と同じ機能を備えている。
...
|is_same_xxx                   |実装方法                                               |
|------------------------------|-------------------------------------------------------|
|[is_same_f](template_meta_programming.md#SS_4_3_2_1)              |関数テンプレートのオーバーロード                       |
|[is_same_v](template_meta_programming.md#SS_4_3_2_2)              |定数テンプレートの特殊化                               |
|[is_same_s](template_meta_programming.md#SS_4_3_2_3)              |クラステンプレートの特殊化                             |
|[is_same_sfinae_f](template_meta_programming.md#SS_4_3_2_4)       |SFINAEと関数テンプレート/関数のオーバーロード          |
|[is_same_sfinae_s](template_meta_programming.md#SS_4_3_2_5)       |SFINAEとクラステンプレートの特殊化                     |
|[same_as](template_meta_programming.md#SS_4_3_2_6)                |[コンセプト](core_lang_spec.md#SS_6_11_3)よるis_same_sfinae_sと同一の機能      |
|[is_same_templ](template_meta_programming.md#SS_4_3_2_7)          |テンプレートテンプレートパラメータ                     |
|[IsSameSomeOf](template_meta_programming.md#SS_4_3_2_8)           |パラメータパックと再帰                                 |
|[OneOf](template_meta_programming.md#SS_4_3_2_9)                  |IsSameSomeOfをコンセプトに                             |
...
「[is_same_v](template_meta_programming.md#SS_4_3_2_2)の実装」と同様に単純であるため、解説は不要だろう。 単体テストは以下のようになる。
...
「[is_void_sfinae_s](template_meta_programming.md#SS_4_3_1_4)の実装」とほぼ同様であるため、解説は不要だろう。 
単体テストは以下のようになる。
...
[SFINAE](core_lang_spec.md#SS_6_11_1)による[is_same_sfinae_s](template_meta_programming.md#SS_4_3_2_5)の難解なコードを[コンセプト](core_lang_spec.md#SS_6_11_3)
...
「[is_same_s](template_meta_programming.md#SS_4_3_2_3)」で紹介した特殊化のテクニックを下記のように使用することができる。
...
* 第1パラメータが第2パラメータ以降で指定された型のどれかと一致する
  SameAsSomeOfという名前の[コンセプト](core_lang_spec.md#SS_6_11_3)を[畳み込み式](core_lang_spec.md#SS_6_11_5)を使用し定義する
...
Usが1つだった場合、SameAsSomeOfは処理をstd::same_as(「[same_as](template_meta_programming.md#SS_4_3_2_6)」参照)に委譲する。
...
OneOfは、[IsSameSomeOf](template_meta_programming.md#SS_4_3_2_8)同様の機能を持つコンセプトである。
OneOfの実装にはシンプルに記述するための[畳み込み式](core_lang_spec.md#SS_6_11_5)を使用した。
...
|--------------------------------------|---------------------------------------------|
|[AreConvertible](template_meta_programming.md#SS_4_3_3_1)                 |クラステンプレートの特殊化                   |
|[ConvertibleToAll](template_meta_programming.md#SS_4_3_3_3)               |AreConvertibleをコンセプトへ                 |
|[AreConvertibleWithoutNarrowConv](template_meta_programming.md#SS_4_3_3_2)|SFINAEとクラステンプレートの特殊化           |
|[ConvertibleWithoutNarrowing](template_meta_programming.md#SS_4_3_3_4)    |AreConvertibleWithoutNarrowConvをコンセプトへ|
...
「[IsSameSomeOf](template_meta_programming.md#SS_4_3_2_8)の実装」のコードパターンとほぼ同様であるため、解説は不要だろうが、
...
上記を抜粋した下記のコードは「縮小型変換を発生さる{}による初期化は[ill-formed](core_lang_spec.md#SS_6_14_1)になる」
ことをSFINAEに利用している。
...
* [AreConvertibleWithoutNarrowConv](template_meta_programming.md#SS_4_3_3_2)と同様の機能を持つ
* [ConvertibleToAll](template_meta_programming.md#SS_4_3_3_3)と同様構造を持つ
...
|メタ関数名                              |メタ関数の目的                                    |
|----------------------------------------|--------------------------------------------------|
|[exists_void_func_sfinae_f](template_meta_programming.md#SS_4_3_4_1)        |メンバ関数void func()を持つかどうかの判断         |
|[exists_void_func_sfinae_s](template_meta_programming.md#SS_4_3_4_2)        |同上                                              |
|[exists_void_func_sfinae_s2](template_meta_programming.md#SS_4_3_4_3)       |同上                                              |
|[exists_void_func_concept](template_meta_programming.md#SS_4_3_4_4)         |同上。コンセプトによるSFINAEの回避                |
...
|メタ関数名                            |メタ関数の目的                                                     |
|--------------------------------------|-------------------------------------------------------------------|
|[exists_begin/exsits_end](template_meta_programming.md#SS_4_3_4_5)        |SFINAEを使用したstd::begin(T)/std::end(T)が存在するか否かの診断    |
|[Array](template_meta_programming.md#SS_4_3_4_7)                          |型が配列である制約を行うためのコンセプト                           |
|[Beginable/Endable](template_meta_programming.md#SS_4_3_4_8)              |[コンセプト](core_lang_spec.md#SS_6_11_3)を使用したexists_begin/exsits_endを単純化した例   |
|[IsRange](template_meta_programming.md#SS_4_3_4_6)                        |exists_begin/exsits_endを使し、範囲forのオペランドになれるか?の判断|
|[Ranged](template_meta_programming.md#SS_4_3_4_9)                         |機能はIsRangeと同一だが、[コンセプト](core_lang_spec.md#SS_6_11_3)を使用しSFINAEの回避     |
|[Container](template_meta_programming.md#SS_4_3_4_10)                      |Ranged且つ!Arrayをコンテナと便宜的に決めつける                     |
...
|メタ関数名                            |メタ関数の目的                                         |
|--------------------------------------|-------------------------------------------------------|
|[exists_put_to_as_member](template_meta_programming.md#SS_4_3_4_11)        |std::ostream::operator<<(T)が存在するか否かの診断      |
|[exists_put_to_as_non_member](template_meta_programming.md#SS_4_3_4_12)    |operator<<(std::ostream&, T)が存在するか否かの診断     |
|[ExistsPutTo](template_meta_programming.md#SS_4_3_4_13)                    |std::ostream& << Tができるかどうかの診断               |
|[Printable](template_meta_programming.md#SS_4_3_4_14)                      |std::ostream& << Tができるかどうか制約コンセプト       |
...
[exists_void_func_sfinae_s](template_meta_programming.md#SS_4_3_4_2)や[exists_void_func_sfinae_s2](template_meta_programming.md#SS_4_3_4_3)
...
以下に示すように、C++20から導入された[コンセプト](core_lang_spec.md#SS_6_11_3)はこのような問題の軽減につながる。
...
「[exists_void_func_sfinae_s](template_meta_programming.md#SS_4_3_4_2)」
...
* [ill-formed](core_lang_spec.md#SS_6_14_1)ならばill-formedになる
* [well-formed](core_lang_spec.md#SS_6_14_2)ならvoidを生成する
...
の戻り型が配列型の[rvalue](core_lang_spec.md#SS_6_7_1_2)である"int (&&) [3]"となり、
...
そのオブジェクトが[lvalue](core_lang_spec.md#SS_6_7_1_1)(この例ではint (&)[3])であれば、
...
コンセプトを使用し、[exists_begin/exsits_end](template_meta_programming.md#SS_4_3_4_5)をリファクタリングした例を以下に示す。
...
すでにみたようにRangedは[exists_begin/exsits_end](template_meta_programming.md#SS_4_3_4_5)の醜いコードを使用しないことで、
...
「[exists_void_func_sfinae_f](template_meta_programming.md#SS_4_3_4_1)の実装」と同様のパターンを使用したので解説は不要だろう。
...
「[exists_begin/exsits_end](template_meta_programming.md#SS_4_3_4_5)や[exists_put_to_as_member](template_meta_programming.md#SS_4_3_4_11)の実装」
...
「[IsRange](template_meta_programming.md#SS_4_3_4_6)の実装」に影響されて、一旦このように実装したが、先に書いた通り、
...
* [SFINAE](core_lang_spec.md#SS_6_11_1)を利用した[ExistsPutTo](template_meta_programming.md#SS_4_3_4_13)は複雑で醜いため、リファクタリングする。
...
これ以降は、[ExistsPutTo](template_meta_programming.md#SS_4_3_4_13)ではなくPrintableを使用する。
...
この特殊化のテンプレートパラメータの制約にはすでに開発したコンセプト[Container](template_meta_programming.md#SS_4_3_4_10)を使用する。
...
「[安全な配列型コンテナ](template_meta_programming.md#SS_4_2_3)」で断念したSafeArray2の開発を再開する前に、
SafeArray2の要件をまとめると、
...
縮小型変換が起こるか否かの場合分けは、
コンセプト[ConvertibleWithoutNarrowing](template_meta_programming.md#SS_4_3_3_4)を使用したSFINAEで実現させることができる。
...
このような記述が必要になる。
一方で[コンセプト](core_lang_spec.md#SS_6_11_3)を使用したC++20スタイルのSFINAEの可読性の高さを実感できただろう。
...
「[安全なvector](template_meta_programming.md#SS_4_2_2)」、「[安全な配列型コンテナ](template_meta_programming.md#SS_4_2_3)」等の中で、
...
(「[is_same_templ](template_meta_programming.md#SS_4_3_2_7)」参照)を使用し下記のように書ける。
...
「[メタ関数のテクニック](template_meta_programming.md#SS_4_3)」で紹介したSFINAEによりこの問題を回避できるが、
ここでも、すでにみてきた[コンセプト](core_lang_spec.md#SS_6_11_3)による制約によりこの問題に対処する。
...
「[Nstd::SafeIndexのoperator\<\<の開発](template_meta_programming.md#SS_4_4_3)」で定義したNstd::operator\<\<の構造は、
...
実際、現在使用中の[g++](cpp_idioms.md#SS_8_10_1)ではこのコードはコンパイルできず、
...
[浮動小数点型](core_lang_spec.md#SS_6_1_12)を頻繁に使用するソフトウェアの開発を行うに場合、
ソースコードの中で、場当たり的に浮動小数点型を使用すると、
[浮動小数点の誤差](core_lang_spec.md#SS_6_1_12_2)や[浮動小数点の演算エラー](core_lang_spec.md#SS_6_1_12_4)
にまつわるバグの修正に多くの工数をロスしてしまうことになる。

これらの課題に対処するため、この節は浮動小数点演算によるバグを未然に防ぎ、
精度を確保するための機能を提供することを目的としている。
[浮動小数点の比較](template_meta_programming.md#SS_4_5_1)方法や、
浮動小数点を避けて高精度な演算を実現する[固定小数点クラス](template_meta_programming.md#SS_4_5_2)および[有理数クラス](template_meta_programming.md#SS_4_5_4)を導入し、
...
[固定小数点クラス](template_meta_programming.md#SS_4_5_2)のようなクラス定義には、以下に示すようにユーザ定義リテラルを定義し、
...
[固定小数点クラス](template_meta_programming.md#SS_4_5_2)では精度が足りず、浮動小数点を使用したくない場合、
...
[固定小数点クラス](template_meta_programming.md#SS_4_5_2)に対して、[固定小数点リテラル](template_meta_programming.md#SS_4_5_3)を定義したように、
使い勝手のよい環境をユーザに提供するために、
[有理数クラス](template_meta_programming.md#SS_4_5_4)に対して、有理数リテラルを定義するべきである。
...
それは 「[コンテナ用Nstd::operator\<\<の開発](template_meta_programming.md#SS_4_4_4)」で示したコードそのものであるため、
...
「[Nstdライブラリの開発](template_meta_programming.md#SS_4_2)」で示した依存関係も維持されており、
...
[forwardingリファレンス](core_lang_spec.md#SS_6_8_3)を使って下記のように書き直した。
...
この原因は、
「関数が受け取った[rvalue](core_lang_spec.md#SS_6_7_1_2)リファレンスは、
その関数から別の関数に受け渡される時に[lvalue](core_lang_spec.md#SS_6_7_1_1)リファレンスとして扱われる」からである。
...
[畳み込み式](core_lang_spec.md#SS_6_11_5)を使用した下記の実装の方がより明確である。
...
(T const&&はconstな[rvalue](core_lang_spec.md#SS_6_7_1_2)リファレンスである)、
forwardingリファレンスが[lvalue](core_lang_spec.md#SS_6_7_1_1)リファレンスであった場合は、
...
なお、forwardingリファレンスは、[リファレンスcollapsing](core_lang_spec.md#SS_6_8_6)の一機能としても理解できる。
...
「[Nstd::SafeIndexの開発](template_meta_programming.md#SS_4_4_2)」や「[コンテナ用Nstd::operator\<\<の開発](template_meta_programming.md#SS_4_4_4)」
...
この記法は[ジェネリックラムダ](core_lang_spec.md#SS_6_11_6)と呼ばれる。
...
ここで示した関数テンプレートは、デザインパターン[Visitor](design_pattern.md#SS_3_21)の例であり、
...
このコードパターンについては、
「[CRTP(curiously recurring template pattern)](design_pattern.md#SS_3_22)」で説明している。
...
[ADL](core_lang_spec.md#SS_6_12_5)(実引数依存探索)により、Appもis_equalの[name lookup](core_lang_spec.md#SS_6_12_2)の対象になるからである。
...
* [ジェネリックすぎるテンプレートを書かない](template_meta_programming.md#SS_4_7_4_1)
* [ADLが本当に必要でない限り名前を修飾する](template_meta_programming.md#SS_4_7_4_2)
* [ADL Firewallを使う](template_meta_programming.md#SS_4_7_4_3)
...
[ジェネリックラムダによる関数内での関数テンプレートの定義](template_meta_programming.md#SS_4_7_2)を行えばよい。
...
([two phase name lookup](core_lang_spec.md#SS_6_12_3)の一回目のname lookupでfがバインドされるため)。
...
[ADL](core_lang_spec.md#SS_6_12_5)や[two phase name lookup](core_lang_spec.md#SS_6_12_3)が絡む場合ならなおさらである。

こういう意図しない[name lookup](core_lang_spec.md#SS_6_12_2)に備えるためには、
修飾されていない識別子を使わないこと、つまり、
識別子には、名前空間、クラス名、this->等による修飾を施すことが重要である。

ただし、「[コンテナ用Nstd::operator\<\<の開発](template_meta_programming.md#SS_4_4_4)」で示したコード等にはADLが欠かせないため、
...
上記のApp::ToStringは「[コンテナ用Nstd::operator\<\<の開発](template_meta_programming.md#SS_4_4_4)」のコードを使用し、
...
XY型オブジェクトを引数にした関数呼び出しによる[関連名前空間](core_lang_spec.md#SS_6_12_6)は、
...
「[Nstdライブラリの開発](template_meta_programming.md#SS_4_2)」等で行ったメタ関数の実装は、
...
等によるコンパイルエラーとの戦いである。
また、これをクリアしても[two phase name lookup](core_lang_spec.md#SS_6_12_3)や[ADL](core_lang_spec.md#SS_6_12_5)が次の関門になる。
これには、デバッガのステップ実行が強力な武器となるが、
型を文字列に変換する関数があればこれもまた強力な武器となる。

以下に示すNstd::Type2Strは、「[Nstdライブラリの開発](template_meta_programming.md#SS_4_2)」等で実際に使用したそのような関数である。
...
残念なことにこの関数は非標準であるため、
それを使っているNstd::Inner\_::demangleは[g++](cpp_idioms.md#SS_8_10_1)/[clang++](cpp_idioms.md#SS_8_10_2)
でなければコンパイルできないだろう。
...
[暗黙の型変換](core_lang_spec.md#SS_6_6_2_2)を利用した文字列リテラルからStaticStringオブジェクトへの変換は、
...
「[静的な文字列オブジェクト](template_meta_programming.md#SS_4_7_6)」で見たように、
...
「[RAII(scoped guard)](design_pattern.md#SS_3_10)でも示したScopedGuardの実装を下記する。
...
* [name lookup](core_lang_spec.md#SS_6_12_2)には複雑なルールが適用されるため、非直感的なバインドが行われる場合がある。
  従って、テンプレートライブラリの開発には単体テストは必須である。

* 使用しているコンパイラが[two phase name lookup](core_lang_spec.md#SS_6_12_3)をサポートしているか否かに気を付ける。
  それがオプションである場合は、two phase name lookupを活性化させる。

* 関数型マクロはそれ以外に実装方法がない時のみに使用する
  。

* 可変長引数を持つ関数の実装には[パラメータパック](core_lang_spec.md#SS_6_11_4)を使う。

* 処理速度や関数のリターンの型に影響する場合があるため、
  パラメータパックの処理の順番に気を付ける(「[前から演算するパラメータパック](template_meta_programming.md#SS_4_1_3_2)」参照)。

* [ADL](core_lang_spec.md#SS_6_12_5)を利用しない場合、テンプレートで使う識別子は名前空間名やthis->等で修飾する
  (「[意図しないname lookupの防止](template_meta_programming.md#SS_4_7_4)」参照)。
...
* テンプレートのインターフェースではないが、実装の都合上ヘッダファイルに記述する定義は、
  ...
  外部から参照しない(「[is_void_sfinae_f](template_meta_programming.md#SS_4_3_1_3)の実装」参照)。

* [forwardingリファレンス](core_lang_spec.md#SS_6_8_3)の実際の型がlvalueリファレンスであるならば、
  constなlvalueリファレンスとして扱う
  。

* ユニバーサルリファレンス引数を他の関数に渡すのであれば、std::forwardを使う
  (「[forwardingリファレンス](core_lang_spec.md#SS_6_8_3)」、「[forwardingリファレンスとstd::forward](template_meta_programming.md#SS_4_7_1)」参照)。

* 関数テンプレートとその特殊化はソースコード上なるべく近い位置で定義する
  (「[two phase name lookup](core_lang_spec.md#SS_6_12_3)」参照)。

* [two phase name lookup](core_lang_spec.md#SS_6_12_3)により意図しない副作用が発生する可能性があるため、
...
* 関数テンプレートのオーバーロードと特殊化の[name lookup](core_lang_spec.md#SS_6_12_2)の優先度に気を付ける。
...
* forwardingリファレンスを持つ関数テンプレートをオーバーロードしない。
  「[forwardingリファレンスとstd::forward](template_meta_programming.md#SS_4_7_1)」で述べたように、
...
* テンプレートに関数型オブジェクトを渡す場合、リファレンスの付け忘れに気を付ける
  (「[関数型をテンプレートパラメータで使う](template_meta_programming.md#SS_4_7_7)」
  参照)。

* 意図しないテンプレートパラメータによるインスタンス化の防止や、
  コンパイルエラーを解読しやすくするために、適切にstatic_assert使うことは重要であるが、
  static_assertによるテンプレートパラメータの制約よりも、[コンセプト](core_lang_spec.md#SS_6_11_3)による制約を優先する。

* ランタイム時の処理を削減する、static_assertを適切に用いる等の目的のために、
  関数テンプレートには適切にconstexprを付けて宣言する
  (「[コンテナ用Nstd::operator\<\<の開発](template_meta_programming.md#SS_4_4_4)」
  、「[constexpr関数](core_lang_spec.md#SS_6_5_3)」
  参照)。
