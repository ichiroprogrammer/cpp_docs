...
&emsp;&emsp; [Nstdライブラリの開発3](template_meta_programming.md#SS_4_5)  
...
Logging::Logger::set_innerでのApp::operator<<の[name lookup](term_explanation.md#SS_7_4_2)について考えてみることにする。
...
その認識とは、「テンプレート内の識別子の[name lookup](term_explanation.md#SS_7_4_2)は、
それがインスタンス化される時に行われる」というものであり、これにより
「 Logging::Loggerのname lookupは単体テスト内で行われる。
それはApp::operator<<宣言後であるためコンパイルできる」と考えることができるが、
[two phase name lookup](term_explanation.md#SS_7_4_3)で行われるプロセスと反するため誤りである。
...
[ADL](term_explanation.md#SS_7_4_5)(実引数依存探索)が働く。
また、Logging::Logger::set_inner(x)はテンプレートであるため、[two phase name lookup](term_explanation.md#SS_7_4_3)
が使用される。
...
instsの[関連名前空間](term_explanation.md#SS_7_4_6)もstdであり、Appではない。
その結果App::operator<<は発見できず、このようなエラーになった。

LOGGERからApp::operator<<を使う場合の単体テストは下記のようになるが、
[ADL](term_explanation.md#SS_7_4_5)によってLogging::Logger::set_inner(ints)内に導入される名前空間はstdのみであり、
前記単体テスト同様にコンパイルできない。
...
「[ADL](term_explanation.md#SS_7_4_5)によってLogging::Logger::set_inner(ints)内に導入される名前空間はstdである」
...
上記コードは[two phase name lookup](term_explanation.md#SS_7_4_3)等の効果により、想定通りコンパイルできるが、
...
このドキュメントで使用している[g++](term_explanation.md#SS_7_13_1)ではこのコードはコンパイルでき、
動作も問題ないように思われるが、[clang++](term_explanation.md#SS_7_13_2)では以下のようなエラーが発生し、コンパイルできない。
...
この理由は「[two phase name lookup](term_explanation.md#SS_7_4_3)」の後半で詳しく解説したので、ここでは繰り返さないが、
...
* operator<<(std::ostream& os, App::Ints_t const& ints)
  ...
  (「[割れ窓理論](term_explanation.md#SS_7_15_1)」参照)になってしまうかもしれない)。
...
これまで述べてきたようにApp::Inst_tの[関連名前空間](term_explanation.md#SS_7_4_6)がAppではなく、stdになってしまうからである。

これを回避するためにはその原因を取り払えばよく、
つまり、App::Inst_tの[関連名前空間](term_explanation.md#SS_7_4_6)がAppになるようにすればよい。
...
* using宣言によりstd::vectorのすべてのコンストラクタをApp::Ints_tに導入
  (「[継承コンストラクタ](term_explanation.md#SS_6_5_2)」参照)

としているため、エイリアスで宣言されたInts_tと等価である。
C++03では、[継承コンストラクタ](term_explanation.md#SS_6_5_2)が使えなかったため、
上記のような構造体を定義するためには、
...
コードクローンや、[車輪の再発明](term_explanation.md#SS_7_15_2)による開発効率の低下につながることがある。
...
これを初期化するためには初期化子リスト
(「[初期化子リストコンストラクタ](term_explanation.md#SS_6_5_1)」、
「[一様初期化](term_explanation.md#SS_6_5_6)」参照)を用いるのが良い。
...
* [SFINAE](term_explanation.md#SS_7_5_1)
...
* std::enable_if\<true>::typeは[well-formed](term_explanation.md#SS_7_12_5)
* std::enable_if\<false>::typeは[ill-formed](term_explanation.md#SS_7_12_4)
...
std::enable_ifのこの特性と後述する[SFINAE](term_explanation.md#SS_7_5_1)により、
...
[SFINAE](term_explanation.md#SS_7_5_1)を使用した関数テンプレートis_void_sfinae_fの実装は以下のようになる。
...
であるため、Tがvoidの時のみ[name lookup](term_explanation.md#SS_7_4_2)の対象になる。
...
であるため、Tが非voidの時のみ[name lookup](term_explanation.md#SS_7_4_2)の対象になる。
...
[SFINAE](term_explanation.md#SS_7_5_1)を使用したクラステンプレートis_void_sfinae_sの実装は以下のようになる。
...
is_void_sfinae_sの特殊化が[name lookup](term_explanation.md#SS_7_4_2)の対象の中に見つからなかった場合、
これが使われる。
...
[std::enable_if](template_meta_programming.md#SS_4_3_1_4)による[SFINAE](term_explanation.md#SS_7_5_1)とクラステンプレートの特殊化を使用した
...
|[same_as](template_meta_programming.md#SS_4_3_3_6)                |[コンセプト](term_explanation.md#SS_7_5_2)よるis_same_sfinae_sと同一の機能      |
...
[SFINAE](term_explanation.md#SS_7_5_1)による[is_same_sfinae_s](template_meta_programming.md#SS_4_3_3_5)の難解なコードを[コンセプト](term_explanation.md#SS_7_5_2)
...
* 第1パラメータが第2パラメータ以降で指定された型のどれかと一致する
  SameAsSomeOfという名前の[コンセプト](term_explanation.md#SS_7_5_2)を[畳み込み式](term_explanation.md#SS_7_5_3)を使用し定義する
...
OneOfの実装にはシンプルに記述するための[畳み込み式](term_explanation.md#SS_7_5_3)を使用した。
...
上記を抜粋した下記のコードは「縮小型変換を発生さる{}による初期化は[ill-formed](term_explanation.md#SS_7_12_4)になる」
ことをSFINAEに利用している。
...
|[Beginable/Endable](template_meta_programming.md#SS_4_3_5_8)              |[コンセプト](term_explanation.md#SS_7_5_2)を使用したexists_begin/exsits_endを単純化した例   |
|[IsRange](template_meta_programming.md#SS_4_3_5_6)                        |exists_begin/exsits_endを使し、範囲forのオペランドになれるか?の判断|
|[Ranged](template_meta_programming.md#SS_4_3_5_9)                         |機能はIsRangeと同一だが、[コンセプト](term_explanation.md#SS_7_5_2)を使用しSFINAEの回避     |
...
以下に示すように、C++20から導入された[コンセプト](term_explanation.md#SS_7_5_2)はこのような問題の軽減につながる。
...
* [ill-formed](term_explanation.md#SS_7_12_4)ならばill-formedになる
* [well-formed](term_explanation.md#SS_7_12_5)ならvoidを生成する
...
の戻り型が配列型の[rvalue](term_explanation.md#SS_7_8_3)である"int (&&) [3]"となり、
...
そのオブジェクトが[lvalue](term_explanation.md#SS_7_8_2)(この例ではint (&)[3])であれば、
...
* [SFINAE](term_explanation.md#SS_7_5_1)を利用した[ExistsPutTo](template_meta_programming.md#SS_4_3_5_13)は複雑で醜いため、リファクタリングする。
...
このような記述が必要になる。
一方で[コンセプト](term_explanation.md#SS_7_5_2)を使用したC++20スタイルのSFINAEの可読性の高さを実感できただろう。
...
ここでも、すでにみてきた[コンセプト](term_explanation.md#SS_7_5_2)による制約によりこの問題に対処する。
...
実際、現在使用中の[g++](term_explanation.md#SS_7_13_1)ではこのコードはコンパイルできず、
...
[ユニバーサルリファレンス](term_explanation.md#SS_7_9_1)を使って下記のように書き直した。
...
この原因は、
「関数が受け取った[rvalue](term_explanation.md#SS_7_8_3)リファレンスは、
その関数から別の関数に受け渡される時に[lvalue](term_explanation.md#SS_7_8_2)リファレンスとして扱われる」からである。
...
[畳み込み式](term_explanation.md#SS_7_5_3)を使用した下記の実装の方がより明確である。
...
(T const&&はconstな[rvalue](term_explanation.md#SS_7_8_3)リファレンスである)、
ユニバーサルリファレンスが[lvalue](term_explanation.md#SS_7_8_2)リファレンスであった場合は、
...
なお、ユニバーサルリファレンスは、[リファレンスcollapsing](term_explanation.md#SS_7_9_4)の一機能としても理解できる。
...
この記法は[ジェネリックラムダ](term_explanation.md#SS_7_5_4)と呼ばれる。
...
[ADL](term_explanation.md#SS_7_4_5)(実引数依存探索)により、Appもis_equalの[name lookup](term_explanation.md#SS_7_4_2)の対象になるからである。
...
* [std::enable_if](template_meta_programming.md#SS_4_3_1_4)や[コンセプト](term_explanation.md#SS_7_5_2)等を使用してテンプレートに適用できる型を制約する
...
([two phase name lookup](term_explanation.md#SS_7_4_3)の一回目のname lookupでfがバインドされるため)。
...
[ADL](term_explanation.md#SS_7_4_5)や[two phase name lookup](term_explanation.md#SS_7_4_3)が絡む場合ならなおさらである。

こういう意図しない[name lookup](term_explanation.md#SS_7_4_2)に備えるためには、
...
XY型オブジェクトを引数にした関数呼び出しによる[関連名前空間](term_explanation.md#SS_7_4_6)は、
...
等によるコンパイルエラーとの戦いである。
また、これをクリアしても[two phase name lookup](term_explanation.md#SS_7_4_3)や[ADL](term_explanation.md#SS_7_4_5)が次の関門になる。
...
残念なことにこの関数は非標準であるため、
それを使っているNstd::Inner\_::demangleは[g++](term_explanation.md#SS_7_13_1)/[clang++](term_explanation.md#SS_7_13_2)
でなければコンパイルできないだろう。
...
[非explitなコンストラクタによる暗黙の型変換](term_explanation.md#SS_6_5_4)
...
* [name lookup](term_explanation.md#SS_7_4_2)には複雑なルールが適用されるため、非直感的なバインドが行われる場合がある。
  従って、テンプレートライブラリの開発には単体テストは必須である。

* 使用しているコンパイラが[two phase name lookup](term_explanation.md#SS_7_4_3)をサポートしているか否かに気を付ける。
...
* [ADL](term_explanation.md#SS_7_4_5)を利用しない場合、テンプレートで使う識別子は名前空間名やthis->等で修飾する
...
* ユニバーサルリファレンス引数を他の関数に渡すのであれば、std::forwardを使う
  (「[ユニバーサルリファレンス](term_explanation.md#SS_7_9_1)」、「[ユニバーサルリファレンスとstd::forward](template_meta_programming.md#SS_4_7_1)」参照)。

* 関数テンプレートとその特殊化はソースコード上なるべく近い位置で定義する
  (「[two phase name lookup](term_explanation.md#SS_7_4_3)」参照)。

* [two phase name lookup](term_explanation.md#SS_7_4_3)により意図しない副作用が発生する可能性があるため、
...
* 関数テンプレートのオーバーロードと特殊化の[name lookup](term_explanation.md#SS_7_4_2)の優先度に気を付ける。
...
* ユニバーサルリファレンスを持つ関数テンプレートをオーバーロードしない。
  「[ユニバーサルリファレンスとstd::forward](template_meta_programming.md#SS_4_7_1)」で述べたように、
...
* 意図しないテンプレートパラメータによるインスタンス化の防止や、
  ...
  static_assertによるテンプレートパラメータの制約よりも、[コンセプト](term_explanation.md#SS_7_5_2)による制約を優先する。
...
* ランタイム時の処理を削減する、static_assertを適切に用いる等の目的のために、
  ...
  、「[constexpr関数](term_explanation.md#SS_6_4_2)」
  参照)。
