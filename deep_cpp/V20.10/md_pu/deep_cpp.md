...
&emsp;&emsp; [Templateメソッド](#SS_3_16)  
...
ヒープ以外のメモリからnewするための[プレースメントnew](#SS_5_2_3)を使用しているため、
...
(「[Named Constructor](#SS_3_18)」参照)と呼ばれる。
...
この性質は、[Factory](#SS_3_17)や[Named Constructor](#SS_3_18)の実装には不可欠であるが、
...
DI(「[DI(dependency injection)](#SS_3_12)」参照)と組み合わせて使われることが多い。

「[Templateメソッド](#SS_3_16)」の例にFactoryを適用したソースコードを下記する。
...
Named Connstructorは、[Singleton](#SS_3_13)のようなオブジェクトを複数、生成するためのパターンである。
...
次に示したのは「[Factory](#SS_3_17)」の例にこのパターンを適応したコードである。
...
あるいは、[DI(dependency injection)](#SS_3_12)と組み合わせて、下記のような改善も有用である。
...
これを改めるためには、「[CRTP(curiously recurring template pattern)](#SS_3_22)」が必要になる。

次に示すソースコードはVisitorとは関係がないが、
FileEntityVisitorから派生するクラスを下記クラス図が示すように改善することで、
単体テストが容易になる例である(「[DI(dependency injection)](#SS_3_12)」参照)。
...
このパターンを用いて、「[Visitor](#SS_3_21)」のFileEntityの3つの派生クラスが持つコードクローン
...
GUIアプリケーションを[MVC](#SS_3_24)で実装する場合のModelがSubjectであり、
ViewがObserverNである。
...
ViewはModelの[Observer](#SS_3_23)であるため、ModelはViewへ依存しない。
...
これは避けるべきなので、「[Singleton](#SS_3_13)」で述べた構造を導入すると、
...
このドキュメントで使用している[g++](#SS_6_20_1)ではこのコードはコンパイルでき、
動作も問題ないように思われるが、[clang++](#SS_6_20_2)では以下のようなエラーが発生し、コンパイルできない。
...
* operator<<(std::ostream& os, App::Ints_t const& ints)
  ...
  (「[割れ窓理論](#SS_6_22_1)」参照)になってしまうかもしれない)。
...
コードクローンや、[車輪の再発明](#SS_6_22_2)による開発効率の低下につながることがある。
...
* std::enable_if\<true>::typeは[well-formed](#SS_6_19_6)
* std::enable_if\<false>::typeは[ill-formed](#SS_6_19_5)
...
上記を抜粋した下記のコードは「縮小型変換を発生さる{}による初期化は[ill-formed](#SS_6_19_5)になる」
ことをSFINAEに利用している。
...
* [ill-formed](#SS_6_19_5)ならばill-formedになる
* [well-formed](#SS_6_19_6)ならvoidを生成する
...
の戻り型が配列型の[rvalue](#SS_6_14_1_2)である"int (&&) [3]"となり、
...
そのオブジェクトが[lvalue](#SS_6_14_1_1)(この例ではint (&)[3])であれば、
...
実際、現在使用中の[g++](#SS_6_20_1)ではこのコードはコンパイルできず、
...
この原因は、
「関数が受け取った[rvalue](#SS_6_14_1_2)リファレンスは、
その関数から別の関数に受け渡される時に[lvalue](#SS_6_14_1_1)リファレンスとして扱われる」からである。
...
ここで示した関数テンプレートは、デザインパターン[Visitor](#SS_3_21)の例であり、
...
このコードパターンについては、
「[CRTP(curiously recurring template pattern)](#SS_3_22)」で説明している。
...
残念なことにこの関数は非標準であるため、
それを使っているNstd::Inner\_::demangleは[g++](#SS_6_20_1)/[clang++](#SS_6_20_2)
でなければコンパイルできないだろう。
...
「[RAII(scoped guard)](#SS_3_10)でも示したScopedGuardの実装を下記する。
...
[フリースタンディング環境](#SS_6_21_2)では、sbrkのようなシステムコールは存在しないため、
...
* その他のクラスに対しては、[Singleton](#SS_3_13)や[Named Constructor](#SS_3_18)と同様な静的な方法を用いる
...
これを避けるためには、
[CRTP(curiously recurring template pattern)](#SS_3_22)
...
コンパイラに[g++](#SS_6_20_1)や[clang++](#SS_6_20_2)を使っている場合、
...
&emsp;&emsp;&emsp;&emsp; [lvalue](#SS_6_14_1_1)  
...
&emsp;&emsp;&emsp;&emsp; [lvalueからの代入](#SS_6_15_2_1)  
&emsp;&emsp;&emsp;&emsp; [rvalueからの代入](#SS_6_15_2_2)  
&emsp;&emsp;&emsp;&emsp; [std::move(lvalue)からの代入](#SS_6_15_2_3)  
...
&emsp;&emsp;&emsp; [lvalue修飾](#SS_6_16_2)  

&emsp;&emsp; [エクセプション安全性の保証](#SS_6_17)  
&emsp;&emsp;&emsp; [no-fail保証](#SS_6_17_1)  
&emsp;&emsp;&emsp; [強い安全性の保証](#SS_6_17_2)  
&emsp;&emsp;&emsp; [基本的な安全性の保証](#SS_6_17_3)  
&emsp;&emsp;&emsp; [noexcept](#SS_6_17_4)  
&emsp;&emsp;&emsp; [exception-unfriendly](#SS_6_17_5)  

&emsp;&emsp; [シンタックス、セマンティクス](#SS_6_18)  
&emsp;&emsp;&emsp; [等価性のセマンティクス](#SS_6_18_1)  
&emsp;&emsp;&emsp; [copyセマンティクス](#SS_6_18_2)  
&emsp;&emsp;&emsp; [moveセマンティクス](#SS_6_18_3)  
&emsp;&emsp;&emsp; [MoveAssignable要件](#SS_6_18_4)  
&emsp;&emsp;&emsp; [CopyAssignable要件](#SS_6_18_5)  

&emsp;&emsp; [C++その他](#SS_6_19)  
&emsp;&emsp;&emsp; [型特性キーワード](#SS_6_19_1)  
&emsp;&emsp;&emsp;&emsp; [alignof](#SS_6_19_1_1)  
&emsp;&emsp;&emsp;&emsp; [alignas](#SS_6_19_1_2)  
&emsp;&emsp;&emsp;&emsp; [addressof](#SS_6_19_1_3)  

&emsp;&emsp;&emsp; [演算子のオペランドの評価順位](#SS_6_19_2)  
&emsp;&emsp;&emsp; [実引数/仮引数](#SS_6_19_3)  
&emsp;&emsp;&emsp; [単純代入](#SS_6_19_4)  
&emsp;&emsp;&emsp; [ill-formed](#SS_6_19_5)  
&emsp;&emsp;&emsp; [well-formed](#SS_6_19_6)  
&emsp;&emsp;&emsp; [未定義動作](#SS_6_19_7)  
&emsp;&emsp;&emsp; [未規定動作](#SS_6_19_8)  
&emsp;&emsp;&emsp; [未定義動作と未規定動作](#SS_6_19_9)  
&emsp;&emsp;&emsp; [被修飾型](#SS_6_19_10)  
&emsp;&emsp;&emsp; [one-definition rule](#SS_6_19_11)  
&emsp;&emsp;&emsp; [ODR](#SS_6_19_12)  
&emsp;&emsp;&emsp; [RVO(Return Value Optimization)](#SS_6_19_13)  
&emsp;&emsp;&emsp; [SSO(Small String Optimization)](#SS_6_19_14)  
&emsp;&emsp;&emsp; [heap allocation elision](#SS_6_19_15)  
&emsp;&emsp;&emsp; [Most Vexing Parse](#SS_6_19_16)  
&emsp;&emsp;&emsp; [トライグラフ](#SS_6_19_17)  

&emsp;&emsp; [C++コンパイラ](#SS_6_20)  
&emsp;&emsp;&emsp; [g++](#SS_6_20_1)  
&emsp;&emsp;&emsp; [clang++](#SS_6_20_2)  

&emsp;&emsp; [ソフトウェア一般](#SS_6_21)  
&emsp;&emsp;&emsp; [ハンドル](#SS_6_21_1)  
&emsp;&emsp;&emsp; [フリースタンディング環境](#SS_6_21_2)  
&emsp;&emsp;&emsp; [サイクロマティック複雑度](#SS_6_21_3)  
&emsp;&emsp;&emsp; [凝集度](#SS_6_21_4)  
&emsp;&emsp;&emsp;&emsp; [凝集度の欠如](#SS_6_21_4_1)  
&emsp;&emsp;&emsp;&emsp; [LCOMの評価基準](#SS_6_21_4_2)  

&emsp;&emsp;&emsp; [Spurious Wakeup](#SS_6_21_5)  
&emsp;&emsp;&emsp; [副作用](#SS_6_21_6)  
&emsp;&emsp;&emsp; [is-a](#SS_6_21_7)  
&emsp;&emsp;&emsp; [has-a](#SS_6_21_8)  
&emsp;&emsp;&emsp; [is-implemented-in-terms-of](#SS_6_21_9)  
&emsp;&emsp;&emsp;&emsp; [public継承によるis-implemented-in-terms-of](#SS_6_21_9_1)  
&emsp;&emsp;&emsp;&emsp; [private継承によるis-implemented-in-terms-of](#SS_6_21_9_2)  
&emsp;&emsp;&emsp;&emsp; [コンポジションによる(has-a)is-implemented-in-terms-of](#SS_6_21_9_3)  

&emsp;&emsp; [非ソフトウェア用語](#SS_6_22)  
&emsp;&emsp;&emsp; [割れ窓理論](#SS_6_22_1)  
&emsp;&emsp;&emsp; [車輪の再発明](#SS_6_22_2)  
...
* リソース管理を外部([RAII(scoped guard)](#SS_3_10)クラス)に任せられる場合: ゼロの法則を採用し、特殊メンバ関数を明示的に定義しない。
...
リソース管理を専門とするクラス
(例: 標準ライブラリの[RAII(scoped guard)](#SS_3_10)クラス)に任せる設計ガイドラインを指す。
...
* [等価性のセマンティクス](#SS_6_18_1)
* [copyセマンティクス](#SS_6_18_2)
...
各クラスがvtblへのポインタを保持するため、このドキュメントで使用している[g++](#SS_6_20_1)では、
...
このドキュメントのコードのコンパイルに使用している[g++](#SS_6_20_1)/[clang++](#SS_6_20_2)では、
そのような関数は、`abi::__cxa_demangle`である。

`std::type_info::name()`と`abi::__cxa_demangle`を利用して、
オブジェクトの[被修飾型](#SS_6_19_10)名をstd::stringオブジェクトとして取り出す関数とその使用例を以下に示す。
...
[ill-formed](#SS_6_19_5)を使用した場合分けが可能になる。
...
オブジェクトの所有権やその移動、
std::unique_ptr、std::move()、[rvalue](#SS_6_14_1_2)の関係を解説する。
...
下記に示した上記クラスの単体テストにより、
オブジェクトの所有権やその移動、共有、
std::shared_ptr、std::move()、[rvalue](#SS_6_14_1_2)の関係を解説する。
...
* prvalue(「[rvalue](#SS_6_14_1_2)」参照)のライフタイム
...
スライシングが起こった場合、そうならないことが問題である(「[等価性のセマンティクス](#SS_6_18_1)」参照)。
...
サンプルコードを[g++](#SS_6_20_1)と[clang++](#SS_6_20_2)でコンパイルしている)。
...
によるとこの動作は未定義のようだが、
[g++](#SS_6_20_1)/[clang++](#SS_6_20_2)両方ともこのコードを警告なしでコンパイルする)。
...
このテストはエラーとならなければならないが、[g++](#SS_6_20_1)ではパスしてしまう。
2nd name lookupのロジックにバグがあるようである。

有難いことに、[clang++](#SS_6_20_2)では仕様通りこのテストはエラーとなり、
...
[name lookup](#SS_6_10_2)の対象外になる」現象一般を指す通称である
...
「テンプレートのパラメータ置き換えに失敗した([ill-formed](#SS_6_19_5)になった)際に、
...
といったセマンティクス的観点(「[シンタックス、セマンティクス](#SS_6_18)」参照)によるものである。
...
[rvalueリファレンス](#SS_6_15_2)(`T&&`型の変数)はlvalueである。
一方、その初期化に使われる式(例えばstd::move(x))は[xvalue](#SS_6_14_1_3)である。
...
* prvalue(もしくはrvalue)は、
    * アドレス演算子(&)のオペランドになれない。
    * 非constな[lvalueリファレンス](#SS_6_15_1)ではバインドできないが、
      constな[lvalueリファレンス](#SS_6_15_1)や[rvalueリファレンス](#SS_6_15_2)でバインドできる。
  

つまり、prvalueとはいわゆるテンポラリオブジェクトのことである(下記の`std::string{}`で作られるようなオブジェクト)。
多くの場合、prvalueはテンポラリオブジェクトを生成するが、
C++17以降は[RVO(Return Value Optimization)](#SS_6_19_13)により、
...
* [lvalueからの代入](#SS_6_15_2_1)
* [rvalueからの代入](#SS_6_15_2_2)
* [std::move(lvalue)からの代入](#SS_6_15_2_3)



#### lvalueからの代入 <a id="SS_6_15_2_1"></a>
下記コードにより「[lvalue](#SS_6_14_1_1)からの代入」を説明する。
...
下記コードにより「[rvalue](#SS_6_14_1_2)からの代入」を説明する。
...
* 行１  
  「[lvalueからの代入](#SS_6_15_2_1)」の行１と同じである。

* 行２  
  「[lvalueからの代入](#SS_6_15_2_1)」の行２と同じである。

* 行３  
  std::moveは単にrvalueリファレンスへのキャストを行うだけであり、ランタイム時の処理コストは発生しない。
  この例の場合、std::stringがmoveコンストラクタ／move代入演算子を提供しているため、
  下記図のようなバッファの所有が移し替えられるだけである(この代入もmove代入と呼ぶ)。
  この動作は「[rvalueからの代入](#SS_6_15_2_2)の行２の左辺」と同じであり、同様に速度が向上するが、その副作用として、
  str1への代入以外のアクセスは[未規定動作](#SS_6_19_8)であるため、避けるべきである
...
下記GetString0()のような関数が返すオブジェクトの内部メンバに対する[ハンドル](#SS_6_21_1)は、
...
* [no-fail保証](#SS_6_17_1)
* [強い安全性の保証](#SS_6_17_2)
* [基本的な安全性の保証](#SS_6_17_3)

### no-fail保証 <a id="SS_6_17_1"></a>
「no-fail保証」を満たす関数はエクセプションをthrowしない。
no-failを保証する関数は、
[noexcept](#SS_6_17_4)を使用してエクセプションを発生させないことを明示できる。
...
の呼び出しでエクセプションがthrowされると、[未定義動作](#SS_6_19_7)や[未規定動作](#SS_6_19_8)が発生するため、
...
* クラスや関数がその名前から想起できる責務を持っている
    ...
    * [Accessor](#SS_3_5)を実装する関数の名前は、GetXxxやSetXxxになっている
    * コンテナクラスのメンバ関数beginやendは、
      そのクラスが保持する値集合の先頭や最後尾の次を指すイテレータを返す
    * 等

* 「[等価性のセマンティクス](#SS_6_18_1)」を守ってる
* 「[copyセマンティクス](#SS_6_18_2)」を守ってる
* 「[moveセマンティクス](#SS_6_18_3)」を守っている
...
従って、これらのオブジェクトに対して[等価性のセマンティクス](#SS_6_18_1)
...
下記の単体テストから明らかな通り、
「[等価性のセマンティクス](#SS_6_18_1)」で示した最後の例も、copyセマンティクスを満たしていない。
...
* リソース管理   
    必須ではないが、aがポインタ等のリソースを保有している場合、
     move代入後にはそのリソースはcに移動していることが一般的である(「[rvalue](#SS_6_14_1_2)」参照)

* エクセプション安全性  
    [no-fail保証](#SS_6_17_1)をする(noexceptと宣言し、エクセプションをthrowしない)
...
4. 効率性  
   ...
   これは、リソースの複製を避けることで達成される(「[moveセマンティクス](#SS_6_18_3)」参照)。
...
3. 正しいセマンティクス  
   copy代入によって代入元のオブジェクトが変更されてはならない(「[copyセマンティクス](#SS_6_18_2)」参照)。
...
[alignas](#SS_6_19_1_2)、
[alignof](#SS_6_19_1_1)によりコンパイラの標準的な方法でアライメントのコントロールできるようになった。
...
それに対し、C++14までは、演算子のオペランド部分式の評価順序は[未規定動作](#SS_6_19_8)であった。
...
* [well-formed](#SS_6_19_6)(適格)とはプログラムが全ての構文規則・診断対象の意味規則・
  単一定義規則を満たすことである。
...
| 種類            |定義                                                               | 例                               | 結果                           |
|-----------------|-------------------------------------------------------------------|----------------------------------|--------------------------------|
|[未定義動作](#SS_6_19_7)|C++標準が全く保証しない動作                                        | ゼロ除算、配列範囲外アクセス     | 予測不能(クラッシュなど)       |
|[未規定動作](#SS_6_19_8)|C++標準が動作を定めていないが、いくつかの選択肢が許容されている動作| `int8_t` に収まらない値のキャスト| 実装依存(異なるが合理的な動作) |
...
### ハンドル <a id="SS_6_21_1"></a>
CやC++の文脈でのハンドルとは、ポインタかリファレンスを指す。

### フリースタンディング環境 <a id="SS_6_21_2"></a>
...
(ただし、「[Immutable](#SS_3_7)」の観点からは、QuadraticEquation::Set()がない方が良い)。
...
#### 凝集度の欠如 <a id="SS_6_21_4_1"></a>
[凝集度](#SS_6_21_4)の欠如(Lack of Cohesion in Methods/LCOM)とは、
...
[LCOMの評価基準](#SS_6_21_4_2)に具体的な推奨値を示す。

#### LCOMの評価基準 <a id="SS_6_21_4_2"></a>
...
### Spurious Wakeup <a id="SS_6_21_5"></a>
...
kyukanchoとstd::stringの関係を[has-a](#SS_6_21_8)の関係と呼ぶ。


### has-a <a id="SS_6_21_8"></a>
「has-a」の関係は、
...
### is-implemented-in-terms-of <a id="SS_6_21_9"></a>
...
自身の機能を提供する場合に使われる。
[has-a](#SS_6_21_8)の関係は、is-implemented-in-terms-of の関係の一種である。

is-implemented-in-terms-ofは下記の手段1-3に示した方法がある。

*手段1.* [public継承によるis-implemented-in-terms-of](#SS_6_21_9_1)  
*手段2.* [private継承によるis-implemented-in-terms-of](#SS_6_21_9_2)  
*手段3.* [コンポジションによる(has-a)is-implemented-in-terms-of](#SS_6_21_9_3)  
...
手段1は[is-a](#SS_6_21_7)に見え、以下に示すような問題も考慮する必要があるため、
可読性、保守性を劣化させる可能性がある。
...
#### public継承によるis-implemented-in-terms-of <a id="SS_6_21_9_1"></a>
...
すでに述べたようにこの方法は、
[private継承によるis-implemented-in-terms-of](#SS_6_21_9_2)や、
[コンポジションによる(has-a)is-implemented-in-terms-of](#SS_6_21_9_3)
と比べコードがシンプルになる。 

#### private継承によるis-implemented-in-terms-of <a id="SS_6_21_9_2"></a>
...
この方法は、[public継承によるis-implemented-in-terms-of](#SS_6_21_9_1)が持つデストラクタ問題は発生せす、
[is-a](#SS_6_21_7)と誤解してしまう問題も発生しない。


#### コンポジションによる(has-a)is-implemented-in-terms-of <a id="SS_6_21_9_3"></a>
