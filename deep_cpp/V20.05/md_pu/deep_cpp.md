...
* copy代入演算子には、[エクセプション安全性の保証](#SS_6_16)がない。
...
これにより[エクセプション安全性の保証](#SS_6_16)を持つ4関数をコードクローンすることなく実装できる。
...
(「[Named Constructor](#SS_3_17)」参照)と呼ばれる。
...
この性質は、[Factory](#SS_3_16)や[Named Constructor](#SS_3_17)の実装には不可欠であるが、
...
DI(「[DI(dependency injection)](#SS_3_11)」参照)と組み合わせて使われることが多い。

「[Templateメソッド](#SS_3_15)」の例にFactoryを適用したソースコードを下記する。
...
これを改めるためには、「[CRTP(curiously recurring template pattern)](#SS_3_21)」が必要になる。

次に示すソースコードはVisitorとは関係がないが、
FileEntityVisitorから派生するクラスを下記クラス図が示すように改善することで、
単体テストが容易になる例である(「[DI(dependency injection)](#SS_3_11)」参照)。
...
GUIアプリケーションを[MVC](#SS_3_23)で実装する場合のModelがSubjectであり、
ViewがObserverNである。
...
ViewはModelの[Observer](#SS_3_22)であるため、ModelはViewへ依存しない。
...
* クラスの理解が曖昧なC++プログラマの理解を助ける(「[ポリモーフィックなクラス](#SS_6_3_9)」参照)
...
c++20から導入された[コンセプト](#SS_6_11_2)によりジェネリックプログラミングは、
開発容易性、可読性、保守性が大きく向上しため、この章のコード例には、
[コンセプト](#SS_6_11_2)(`concept`, `requires`)を多用した。
...
&emsp;&emsp;&emsp; [パラメータパック](#SS_4_1_3)  
...
&emsp;&emsp;&emsp; [STLのtype_traits](#SS_4_3_1)  
&emsp;&emsp;&emsp; [is_void_xxxの実装](#SS_4_3_2)  
&emsp;&emsp;&emsp; [is_same_xxxの実装](#SS_4_3_3)  
&emsp;&emsp;&emsp; [AreConvertibleXxxの実装](#SS_4_3_4)  
&emsp;&emsp;&emsp; [関数の存在の診断](#SS_4_3_5)  
...
これは避けるべきなので、「[Singleton](#SS_3_12)」で述べた構造を導入すると、
...
なお、上記コードで使用した[std::is_same](#SS_4_3_1_3)は、
...
Logging::Logger::set_innerでのApp::operator<<の[name lookup](#SS_6_10_2)について考えてみることにする。
...
[ADL](#SS_6_10_5)(実引数依存探索)が働く。
また、Logging::Logger::set_inner(x)はテンプレートであるため、[two phase name lookup](#SS_6_10_3)
が使用される。
...
instsの[関連名前空間](#SS_6_10_6)もstdであり、Appではない。
その結果App::operator<<は発見できず、このようなエラーになった。

LOGGERからApp::operator<<を使う場合の単体テストは下記のようになるが、
[ADL](#SS_6_10_5)によってLogging::Logger::set_inner(ints)内に導入される名前空間はstdのみであり、
前記単体テスト同様にコンパイルできない。
...
「[ADL](#SS_6_10_5)によってLogging::Logger::set_inner(ints)内に導入される名前空間はstdである」
...
上記コードは[two phase name lookup](#SS_6_10_3)等の効果により、想定通りコンパイルできるが、
...
このドキュメントで使用している[g++](#SS_6_19_1)ではこのコードはコンパイルでき、
動作も問題ないように思われるが、[clang++](#SS_6_19_2)では以下のようなエラーが発生し、コンパイルできない。
...
この理由は「[two phase name lookup](#SS_6_10_3)」の後半で詳しく解説したので、ここでは繰り返さないが、
...
* operator<<(std::ostream& os, App::Ints_t const& ints)
  ...
  (「[割れ窓理論](#SS_6_21_1)」参照)になってしまうかもしれない)。
...
これまで述べてきたようにApp::Inst_tの[関連名前空間](#SS_6_10_6)がAppではなく、stdになってしまうからである。

これを回避するためにはその原因を取り払えばよく、
つまり、App::Inst_tの[関連名前空間](#SS_6_10_6)がAppになるようにすればよい。
...
* using宣言によりstd::vectorのすべてのコンストラクタをApp::Ints_tに導入
  (「[継承コンストラクタ](#SS_6_5_4)」参照)

としているため、エイリアスで宣言されたInts_tと等価である。
C++03では、[継承コンストラクタ](#SS_6_5_4)が使えなかったため、
上記のような構造体を定義するためには、
...
コードクローンや、[車輪の再発明](#SS_6_21_2)による開発効率の低下につながることがある。
...
これを初期化するためには初期化子リスト
(「[初期化子リストコンストラクタ](#SS_6_5_3)」、
「[一様初期化](#SS_6_5_2)」参照)を用いるのが良い。
...
* std::enable_if\<true>::typeは[well-formed](#SS_6_18_6)
* std::enable_if\<false>::typeは[ill-formed](#SS_6_18_5)
...
であるため、Tがvoidの時のみ[name lookup](#SS_6_10_2)の対象になる。
...
であるため、Tが非voidの時のみ[name lookup](#SS_6_10_2)の対象になる。
...
is_void_sfinae_sの特殊化が[name lookup](#SS_6_10_2)の対象の中に見つからなかった場合、
これが使われる。
...
[std::enable_if](#SS_4_3_1_4)による[SFINAE](#SS_6_11_1)とクラステンプレートの特殊化を使用した
...
|[same_as](#SS_4_3_3_6)                |[コンセプト](#SS_6_11_2)よるis_same_sfinae_sと同一の機能      |
...
[SFINAE](#SS_6_11_1)による[is_same_sfinae_s](#SS_4_3_3_5)の難解なコードを[コンセプト](#SS_6_11_2)
...
* 第1パラメータが第2パラメータ以降で指定された型のどれかと一致する
  SameAsSomeOfという名前の[コンセプト](#SS_6_11_2)を[畳み込み式](#SS_6_11_3)を使用し定義する
...
OneOfの実装にはシンプルに記述するための[畳み込み式](#SS_6_11_3)を使用した。
...
上記を抜粋した下記のコードは「縮小型変換を発生さる{}による初期化は[ill-formed](#SS_6_18_5)になる」
ことをSFINAEに利用している。
...
|[Beginable/Endable](#SS_4_3_5_8)              |[コンセプト](#SS_6_11_2)を使用したexists_begin/exsits_endを単純化した例   |
|[IsRange](#SS_4_3_5_6)                        |exists_begin/exsits_endを使し、範囲forのオペランドになれるか?の判断|
|[Ranged](#SS_4_3_5_9)                         |機能はIsRangeと同一だが、[コンセプト](#SS_6_11_2)を使用しSFINAEの回避     |
...
以下に示すように、C++20から導入された[コンセプト](#SS_6_11_2)はこのような問題の軽減につながる。
...
* [ill-formed](#SS_6_18_5)ならばill-formedになる
* [well-formed](#SS_6_18_6)ならvoidを生成する
...
の戻り型が配列型の[rvalue](#SS_6_14_3)である"int (&&) [3]"となり、
...
そのオブジェクトが[lvalue](#SS_6_14_2)(この例ではint (&)[3])であれば、
...
* [SFINAE](#SS_6_11_1)を利用した[ExistsPutTo](#SS_4_3_5_13)は複雑で醜いため、リファクタリングする。
...
このような記述が必要になる。
一方で[コンセプト](#SS_6_11_2)を使用したC++20スタイルのSFINAEの可読性の高さを実感できただろう。
...
値を型に変換する[std::integral_constant](#SS_4_3_1_2)を使用し、この問題を解決できる。
...
ここでも、すでにみてきた[コンセプト](#SS_6_11_2)による制約によりこの問題に対処する。
...
実際、現在使用中の[g++](#SS_6_19_1)ではこのコードはコンパイルできず、
...
```cpp
    //  example/template_cpp17/nstd_float_ut.cpp 44
...
```cpp
    //  example/template_cpp17/nstd_float_ut.cpp 59
...
```cpp
    //  example/template_cpp17/nstd_float_ut.cpp 74
...
```cpp
    //  example/template_cpp17/nstd_float_ut.cpp 91
...
```cpp
    //  example/template_cpp17/nstd_float_ut.cpp 106
...
```cpp
    //  example/template_cpp17/nstd_float_ut.cpp 125
...
[ユニバーサルリファレンス](#SS_6_15_1)を使って下記のように書き直した。
...
この原因は、
「関数が受け取った[rvalue](#SS_6_14_3)リファレンスは、
その関数から別の関数に受け渡される時に[lvalue](#SS_6_14_2)リファレンスとして扱われる」からである。
...
```cpp
    //  example/template_cpp17/universal_ref_ut.cpp 96
...
```cpp
    //  example/template_cpp17/universal_ref_ut.cpp 124
...
```cpp
    //  example/template_cpp17/universal_ref_ut.cpp 142
...
```cpp
    //  example/template_cpp17/universal_ref_ut.cpp 155
...
```cpp
    //  example/template_cpp17/universal_ref_ut.cpp 171
...
```cpp
    //  example/template_cpp17/universal_ref_ut.cpp 197
...
[畳み込み式](#SS_6_11_3)を使用した下記の実装の方がより明確である。

```cpp
    //  example/template_cpp17/universal_ref_ut.cpp 211
...
(T const&&はconstな[rvalue](#SS_6_14_3)リファレンスである)、
ユニバーサルリファレンスが[lvalue](#SS_6_14_2)リファレンスであった場合は、
constなlvalueリファレンスとして扱うべきである。

従って、下記のようなコードは書くべきではない。

```cpp
    //  example/template_cpp17/universal_ref_ut.cpp 240
...
```cpp
    //  example/template_cpp17/universal_ref_ut.cpp 257
...
```cpp
    //  example/template_cpp17/universal_ref_ut.cpp 292
...
```cpp
    //  example/template_cpp17/universal_ref_ut.cpp 300
...
なお、ユニバーサルリファレンスは、[リファレンスcollapsing](#SS_6_15_4)の一機能としても理解できる。
...
この記法は[ジェネリックラムダ](#SS_6_11_4)と呼ばれる。
...
ここで示した関数テンプレートは、デザインパターン[Visitor](#SS_3_20)の例であり、
...
```cpp
    //  example/template_cpp17/suppress_adl_ut.cpp 37
...
```cpp
    //  example/template_cpp17/suppress_adl_ut.cpp 43
...
[ADL](#SS_6_10_5)(実引数依存探索)により、Appもis_equalの[name lookup](#SS_6_10_2)の対象になるからである。
...
```cpp
    //  example/template_cpp17/suppress_adl_ut.cpp 63
...
```cpp
    //  example/template_cpp17/suppress_adl_ut.cpp 84
...
```cpp
    //  example/template_cpp17/suppress_adl_ut.cpp 93
...
```cpp
    //  example/template_cpp17/suppress_adl_ut.cpp 108
...
```cpp
    //  example/template_cpp17/suppress_adl_ut.cpp 126
...
```cpp
    //  example/template_cpp17/suppress_adl_ut.cpp 153
...
* [std::enable_if](#SS_4_3_1_4)や[コンセプト](#SS_6_11_2)等を使用してテンプレートに適用できる型を制約する
...
```cpp
    //  example/template_cpp17/suppress_adl_ut.cpp 176
...
([two phase name lookup](#SS_6_10_3)の一回目のname lookupでfがバインドされるため)。

```cpp
    //  example/template_cpp17/suppress_adl_ut.cpp 203
...
```cpp
    //  example/template_cpp17/suppress_adl_ut.cpp 213
...
```cpp
    //  example/template_cpp17/suppress_adl_ut.cpp 236
...
[ADL](#SS_6_10_5)や[two phase name lookup](#SS_6_10_3)が絡む場合ならなおさらである。

こういう意図しない[name lookup](#SS_6_10_2)に備えるためには、
...
XY型オブジェクトを引数にした関数呼び出しによる[関連名前空間](#SS_6_10_6)は、
...
等によるコンパイルエラーとの戦いである。
また、これをクリアしても[two phase name lookup](#SS_6_10_3)や[ADL](#SS_6_10_5)が次の関門になる。
...
残念なことにこの関数は非標準であるため、
それを使っているNstd::Inner\_::demangleは[g++](#SS_6_19_1)/[clang++](#SS_6_19_2)
でなければコンパイルできないだろう。
...
[暗黙の型変換](#SS_6_13_1)を利用した文字列リテラルからStaticStringオブジェクトへの変換は、
...
「[RAII(scoped guard)](#SS_3_9)でも示したScopedGuardの実装を下記する。
...
* [name lookup](#SS_6_10_2)には複雑なルールが適用されるため、非直感的なバインドが行われる場合がある。
  従って、テンプレートライブラリの開発には単体テストは必須である。

* 使用しているコンパイラが[two phase name lookup](#SS_6_10_3)をサポートしているか否かに気を付ける。
  それがオプションである場合は、two phase name lookupを活性化させる。

* 関数型マクロはそれ以外に実装方法がない時のみに使用する
  。

* 可変長引数を持つ関数の実装には[パラメータパック](#SS_4_1_3)を使う。

* 処理速度や関数のリターンの型に影響する場合があるため、
  パラメータパックの処理の順番に気を付ける(「[前から演算するパラメータパック](#SS_4_1_3_2)」参照)。

* [ADL](#SS_6_10_5)を利用しない場合、テンプレートで使う識別子は名前空間名やthis->等で修飾する
...
* ユニバーサルリファレンス引数を他の関数に渡すのであれば、std::forwardを使う
  (「[ユニバーサルリファレンス](#SS_6_15_1)」、「[ユニバーサルリファレンスとstd::forward](#SS_4_7_1)」参照)。

* 関数テンプレートとその特殊化はソースコード上なるべく近い位置で定義する
  (「[two phase name lookup](#SS_6_10_3)」参照)。

* [two phase name lookup](#SS_6_10_3)により意図しない副作用が発生する可能性があるため、
...
* 関数テンプレートのオーバーロードと特殊化の[name lookup](#SS_6_10_2)の優先度に気を付ける。
...
&emsp;&emsp; [グローバルnew/deleteのオーバーロード](#SS_5_2)  
&emsp;&emsp;&emsp; [固定長メモリプール](#SS_5_2_1)  
&emsp;&emsp;&emsp; [グローバルnew/deleteのオーバーロードの実装](#SS_5_2_2)  
&emsp;&emsp;&emsp; [プレースメントnew](#SS_5_2_3)  
&emsp;&emsp;&emsp; [デバッグ用イテレータ](#SS_5_2_4)  

&emsp;&emsp; [クラスnew/deleteのオーバーロード](#SS_5_3)  
&emsp;&emsp; [STLコンテナのアロケーター](#SS_5_4)  
&emsp;&emsp;&emsp; [STLコンテナ用アロケータ](#SS_5_4_1)  
&emsp;&emsp;&emsp; [可変長メモリプール](#SS_5_4_2)  
&emsp;&emsp;&emsp; [デバッグ用イテレータ](#SS_5_4_3)  
&emsp;&emsp;&emsp; [エクセプション処理機構の変更](#SS_5_4_4)  

&emsp;&emsp; [Polymorphic Memory Resource(pmr)](#SS_5_5)  
&emsp;&emsp;&emsp; [std::pmr::memory_resource](#SS_5_5_1)  
&emsp;&emsp;&emsp; [std::pmr::polymorphic_allocator](#SS_5_5_2)  
&emsp;&emsp;&emsp; [pool_resource](#SS_5_5_3)  
...
[フリースタンディング環境](#SS_6_20_1)では、sbrkのようなシステムコールは存在しないため、
...
「[グローバルnew/deleteのオーバーロードの実装](#SS_5_2_2)」で示すように、
...
gen_mpool<>については、「[プレースメントnew](#SS_5_2_3)」で説明する。
...
「[グローバルnew/deleteのオーバーロードの実装](#SS_5_2_2)」で使用したgen_mpool<>は、
下記のように定義されている。
...
「[グローバルnew/deleteのオーバーロードの実装](#SS_5_2_2)」で示したコードのロックを、
...
* その他のクラスに対しては、[Singleton](#SS_3_12)や[Named Constructor](#SS_3_17)と同様な静的な方法を用いる
...
これを避けるためには、
[CRTP(curiously recurring template pattern)](#SS_3_21)
...
「[グローバルnew/deleteのオーバーロード](#SS_5_2)」に比べ、優れているように見えてしまうかもしれないが、
...
### デバッグ用イテレータ <a id="SS_5_4_3"></a>
[可変長メモリプール](#SS_5_4_2)を使用すると、
...
「[グローバルnew/deleteのオーバーロードの実装](#SS_5_2_2)」でも述べたように、
...
コンパイラに[g++](#SS_6_19_1)や[clang++](#SS_6_19_2)を使っている場合、
...
* [std::pmr::memory_resource](#SS_5_5_1)  
* [std::pmr::polymorphic_allocator](#SS_5_5_2)  
* [pool_resource](#SS_5_5_3)

### std::pmr::memory_resource <a id="SS_5_5_1"></a>
std::pmr::memory_resourceは、
ユーザー定義のメモリリソースをカスタマイズし、
[std::pmr::polymorphic_allocator](#SS_5_5_2)を通じて利用可能にする[インターフェースクラス](#SS_6_3_13)である。

[可変長メモリプール](#SS_5_4_2)の実装で示したコードとほぼ同様の、
...
[std::pmr::memory_resource](#SS_5_5_1)を基盤とし、
コンテナやアルゴリズムにカスタムメモリアロケーション戦略を容易に適用可能にする。
std::allocatorと異なり、型に依存せず、
ポリモーフィズムを活用してメモリリソースを切り替えられる点が特徴である。

すでに示したmemory_resource_variable([std::pmr::memory_resource](#SS_5_5_1))の単体テストを以下に示すことにより、
polymorphic_allocatorの使用例とする。

```cpp
    //  example/dynamic_memory_allocation/pmr_memory_resource_ut.cpp 217
...
pool_resourceは[std::pmr::memory_resource](#SS_5_5_1)を基底とする下記の2つの具象クラスである。
...
&emsp;&emsp;&emsp; [スコープドenum](#SS_6_2_2)  
&emsp;&emsp;&emsp; [underlying type](#SS_6_2_3)  
&emsp;&emsp;&emsp; [std::byte](#SS_6_2_4)  
&emsp;&emsp;&emsp; [using enum](#SS_6_2_5)  

&emsp;&emsp; [型とインスタンス](#SS_6_3)  
&emsp;&emsp;&emsp; [特殊メンバ関数](#SS_6_3_1)  
&emsp;&emsp;&emsp;&emsp; [ゼロの原則(Rule of Zero)](#SS_6_3_1_1)  
&emsp;&emsp;&emsp;&emsp; [五の原則(Rule of Five)](#SS_6_3_1_2)  

&emsp;&emsp;&emsp; [トリビアル型](#SS_6_3_2)  
&emsp;&emsp;&emsp; [トリビアルに破壊可能な型](#SS_6_3_3)  
&emsp;&emsp;&emsp; [標準レイアウト型](#SS_6_3_4)  
&emsp;&emsp;&emsp; [集成体](#SS_6_3_5)  
&emsp;&emsp;&emsp; [POD](#SS_6_3_6)  
&emsp;&emsp;&emsp; [不完全型](#SS_6_3_7)  
&emsp;&emsp;&emsp; [完全型](#SS_6_3_8)  
&emsp;&emsp;&emsp; [ポリモーフィックなクラス](#SS_6_3_9)  
&emsp;&emsp;&emsp; [オーバーライドとオーバーロードの違い](#SS_6_3_10)  
...
&emsp;&emsp;&emsp; [リスト初期化](#SS_6_5_1)  
&emsp;&emsp;&emsp; [一様初期化](#SS_6_5_2)  
&emsp;&emsp;&emsp; [初期化子リストコンストラクタ](#SS_6_5_3)  
&emsp;&emsp;&emsp; [継承コンストラクタ](#SS_6_5_4)  
&emsp;&emsp;&emsp; [委譲コンストラクタ](#SS_6_5_5)  
&emsp;&emsp;&emsp; [非静的なメンバ変数の初期化](#SS_6_5_6)  
&emsp;&emsp;&emsp;&emsp; [NSDMI](#SS_6_5_6_1)  
&emsp;&emsp;&emsp;&emsp; [初期化子リストでの初期化](#SS_6_5_6_2)  
&emsp;&emsp;&emsp;&emsp; [コンストラクタ内での非静的なメンバ変数の初期値の代入](#SS_6_5_6_3)  

&emsp;&emsp;&emsp; [オブジェクトの所有権](#SS_6_5_7)  
&emsp;&emsp;&emsp;&emsp; [オブジェクトの排他所有](#SS_6_5_7_1)  
&emsp;&emsp;&emsp;&emsp; [オブジェクトの共有所有](#SS_6_5_7_2)  
&emsp;&emsp;&emsp;&emsp; [オブジェクトの循環所有](#SS_6_5_7_3)  
&emsp;&emsp;&emsp;&emsp; [std::weak_ptr](#SS_6_5_7_4)  
...
&emsp;&emsp;&emsp; [2進数リテラル](#SS_6_6_2)  
&emsp;&emsp;&emsp; [数値リテラル](#SS_6_6_3)  
&emsp;&emsp;&emsp; [ワイド文字列](#SS_6_6_4)  
&emsp;&emsp;&emsp; [16進浮動小数点数リテラル](#SS_6_6_5)  
&emsp;&emsp;&emsp; [ユーザー定義リテラル](#SS_6_6_6)  
&emsp;&emsp;&emsp;&emsp; [ユーザ定義リテラル演算子](#SS_6_6_6_1)  
&emsp;&emsp;&emsp;&emsp; [std::string型リテラル](#SS_6_6_6_2)  
&emsp;&emsp;&emsp;&emsp; [std::chronoのリテラル](#SS_6_6_6_3)  
&emsp;&emsp;&emsp;&emsp; [std::complexリテラル](#SS_6_6_6_4)  

&emsp;&emsp;&emsp; [==演算子](#SS_6_6_7)  
&emsp;&emsp;&emsp;&emsp; [メンバ==演算子](#SS_6_6_7_1)  
&emsp;&emsp;&emsp;&emsp; [非メンバ==演算子](#SS_6_6_7_2)  

&emsp;&emsp;&emsp; [比較演算子](#SS_6_6_8)  
&emsp;&emsp;&emsp;&emsp; [std::rel_ops](#SS_6_6_8_1)  
&emsp;&emsp;&emsp;&emsp; [std::tuppleを使用した比較演算子の実装方法](#SS_6_6_8_2)  
&emsp;&emsp;&emsp;&emsp; [<=>演算子](#SS_6_6_8_3)  
&emsp;&emsp;&emsp;&emsp; [三方比較演算子](#SS_6_6_8_4)  
&emsp;&emsp;&emsp;&emsp; [spaceship operator](#SS_6_6_8_5)  
...
&emsp;&emsp;&emsp;&emsp; [co_await](#SS_6_8_1_1)  
&emsp;&emsp;&emsp;&emsp; [co_return](#SS_6_8_1_2)  
&emsp;&emsp;&emsp;&emsp; [co_yield](#SS_6_8_1_3)  
...
&emsp;&emsp;&emsp; [two phase name lookup](#SS_6_10_3)  
&emsp;&emsp;&emsp; [実引数依存探索](#SS_6_10_4)  
&emsp;&emsp;&emsp; [ADL](#SS_6_10_5)  
&emsp;&emsp;&emsp; [関連名前空間](#SS_6_10_6)  
&emsp;&emsp;&emsp; [修飾付き関数呼び出し](#SS_6_10_7)  
&emsp;&emsp;&emsp; [hidden-friend関数](#SS_6_10_8)  
&emsp;&emsp;&emsp; [name-hiding](#SS_6_10_9)  
&emsp;&emsp;&emsp; [ダイヤモンド継承](#SS_6_10_10)  
&emsp;&emsp;&emsp; [仮想継承](#SS_6_10_11)  
&emsp;&emsp;&emsp; [仮想基底](#SS_6_10_12)  
&emsp;&emsp;&emsp; [ドミナンス](#SS_6_10_13)  
&emsp;&emsp;&emsp;&emsp; [ダイヤモンド継承を含まない場合](#SS_6_10_13_1)  
&emsp;&emsp;&emsp;&emsp; [ダイヤモンド継承かつそれが仮想継承でない場合](#SS_6_10_13_2)  
&emsp;&emsp;&emsp;&emsp; [ダイヤモンド継承かつそれが仮想継承である場合](#SS_6_10_13_3)  

&emsp;&emsp;&emsp; [using宣言](#SS_6_10_14)  
&emsp;&emsp;&emsp; [usingディレクティブ](#SS_6_10_15)  

&emsp;&emsp; [template強化機能](#SS_6_11)  
&emsp;&emsp;&emsp; [SFINAE](#SS_6_11_1)  
&emsp;&emsp;&emsp; [コンセプト](#SS_6_11_2)  
&emsp;&emsp;&emsp; [畳み込み式](#SS_6_11_3)  
&emsp;&emsp;&emsp; [ジェネリックラムダ](#SS_6_11_4)  
&emsp;&emsp;&emsp; [クラステンプレートのテンプレート引数の型推論](#SS_6_11_5)  
&emsp;&emsp;&emsp; [テンプレートの型推論ガイド](#SS_6_11_6)  
&emsp;&emsp;&emsp; [CTAD(Class Template Argument Deduction)](#SS_6_11_7)  
&emsp;&emsp;&emsp; [変数テンプレート](#SS_6_11_8)  
&emsp;&emsp;&emsp; [エイリアステンプレート](#SS_6_11_9)  
&emsp;&emsp;&emsp; [constexpr if文](#SS_6_11_10)  
&emsp;&emsp;&emsp; [autoパラメータによる関数テンプレートの簡易定義](#SS_6_11_11)  

&emsp;&emsp; [型推論](#SS_6_12)  
&emsp;&emsp;&emsp; [AAAスタイル](#SS_6_12_1)  
&emsp;&emsp;&emsp; [decltype](#SS_6_12_2)  
&emsp;&emsp;&emsp; [decltype(auto)](#SS_6_12_3)  
&emsp;&emsp;&emsp; [CTAD（Class Template Argument Deduction）](#SS_6_12_4)  
&emsp;&emsp;&emsp; [戻り値型を後置する関数宣言](#SS_6_12_5)  
&emsp;&emsp;&emsp; [関数の戻り値型auto](#SS_6_12_6)  
&emsp;&emsp;&emsp; [後置戻り値型auto](#SS_6_12_7)  

&emsp;&emsp; [explicit](#SS_6_13)  
&emsp;&emsp;&emsp; [暗黙の型変換](#SS_6_13_1)  
&emsp;&emsp;&emsp; [暗黙の型変換抑止](#SS_6_13_2)  
&emsp;&emsp;&emsp; [explicit type operator()](#SS_6_13_3)  
&emsp;&emsp;&emsp; [explicit(COND)](#SS_6_13_4)  

&emsp;&emsp; [expressionと値カテゴリ](#SS_6_14)  
&emsp;&emsp;&emsp; [expression](#SS_6_14_1)  
&emsp;&emsp;&emsp; [lvalue](#SS_6_14_2)  
&emsp;&emsp;&emsp; [rvalue](#SS_6_14_3)  
&emsp;&emsp;&emsp;&emsp; [xvalue](#SS_6_14_3_1)  
&emsp;&emsp;&emsp;&emsp; [prvalue](#SS_6_14_3_2)  

&emsp;&emsp;&emsp; [rvalue修飾](#SS_6_14_4)  
&emsp;&emsp;&emsp; [lvalue修飾](#SS_6_14_5)  
&emsp;&emsp;&emsp; [リファレンス修飾](#SS_6_14_6)  

&emsp;&emsp; [リファレンス](#SS_6_15)  
&emsp;&emsp;&emsp; [ユニバーサルリファレンス](#SS_6_15_1)  
&emsp;&emsp;&emsp; [forwardingリファレンス](#SS_6_15_2)  
&emsp;&emsp;&emsp; [perfect forwarding](#SS_6_15_3)  
&emsp;&emsp;&emsp; [リファレンスcollapsing](#SS_6_15_4)  
&emsp;&emsp;&emsp; [danglingリファレンス](#SS_6_15_5)  
&emsp;&emsp;&emsp; [danglingポインタ](#SS_6_15_6)  

&emsp;&emsp; [エクセプション安全性の保証](#SS_6_16)  
&emsp;&emsp;&emsp; [no-fail保証](#SS_6_16_1)  
&emsp;&emsp;&emsp; [強い安全性の保証](#SS_6_16_2)  
&emsp;&emsp;&emsp; [基本的な安全性の保証](#SS_6_16_3)  
&emsp;&emsp;&emsp; [noexcept](#SS_6_16_4)  
&emsp;&emsp;&emsp; [exception-unfriendly](#SS_6_16_5)  

&emsp;&emsp; [シンタックス、セマンティクス](#SS_6_17)  
&emsp;&emsp;&emsp; [等価性のセマンティクス](#SS_6_17_1)  
&emsp;&emsp;&emsp; [copyセマンティクス](#SS_6_17_2)  
&emsp;&emsp;&emsp; [moveセマンティクス](#SS_6_17_3)  
&emsp;&emsp;&emsp; [MoveAssignable要件](#SS_6_17_4)  
&emsp;&emsp;&emsp; [CopyAssignable要件](#SS_6_17_5)  

&emsp;&emsp; [C++その他](#SS_6_18)  
&emsp;&emsp;&emsp; [型特性キーワード](#SS_6_18_1)  
&emsp;&emsp;&emsp;&emsp; [alignof](#SS_6_18_1_1)  
&emsp;&emsp;&emsp;&emsp; [alignas](#SS_6_18_1_2)  
&emsp;&emsp;&emsp;&emsp; [addressof](#SS_6_18_1_3)  

&emsp;&emsp;&emsp; [演算子のオペランドの評価順位](#SS_6_18_2)  
&emsp;&emsp;&emsp; [実引数/仮引数](#SS_6_18_3)  
&emsp;&emsp;&emsp; [単純代入](#SS_6_18_4)  
&emsp;&emsp;&emsp; [ill-formed](#SS_6_18_5)  
&emsp;&emsp;&emsp; [well-formed](#SS_6_18_6)  
&emsp;&emsp;&emsp; [未定義動作](#SS_6_18_7)  
&emsp;&emsp;&emsp; [未規定動作](#SS_6_18_8)  
&emsp;&emsp;&emsp; [未定義動作と未規定動作](#SS_6_18_9)  
&emsp;&emsp;&emsp; [被修飾型](#SS_6_18_10)  
&emsp;&emsp;&emsp; [one-definition rule](#SS_6_18_11)  
&emsp;&emsp;&emsp; [ODR](#SS_6_18_12)  
&emsp;&emsp;&emsp; [RVO(Return Value Optimization)](#SS_6_18_13)  
&emsp;&emsp;&emsp; [SSO(Small String Optimization)](#SS_6_18_14)  
&emsp;&emsp;&emsp; [heap allocation elision](#SS_6_18_15)  
&emsp;&emsp;&emsp; [Most Vexing Parse](#SS_6_18_16)  
&emsp;&emsp;&emsp; [トライグラフ](#SS_6_18_17)  

&emsp;&emsp; [C++コンパイラ](#SS_6_19)  
&emsp;&emsp;&emsp; [g++](#SS_6_19_1)  
&emsp;&emsp;&emsp; [clang++](#SS_6_19_2)  
...
&emsp;&emsp;&emsp; [車輪の再発明](#SS_6_21_2)  
...
[リスト初期化](#SS_6_5_1)を使うことで、このような変換によるバグの発生を防ぐことができる。
...
このドキュメントでは、[スコープドenum](#SS_6_2_2)に対して、C++03までのenumを非スコープドenum、
...
C++17から導入された[std::byte](#SS_6_2_4)の利便性のため、
...
* [<=>演算子](#SS_6_6_8_3)  
  すべてのメンバが[<=>演算子](#SS_6_6_8_3)での比較可能である場合、`= default`とすることで自動生成される。 
...
* [ゼロの原則(Rule of Zero)](#SS_6_3_1_1)
* [五の原則(Rule of Five)](#SS_6_3_1_2)

この2つの原則(ガイドライン)の使い分けに関しては、

* リソース管理を外部([RAII(scoped guard)](#SS_3_9)クラス)に任せられる場合: ゼロの法則を採用し、特殊メンバ関数を明示的に定義しない。
...
リソース管理を専門とするクラス
(例: 標準ライブラリの[RAII(scoped guard)](#SS_3_9)クラス)に任せる設計ガイドラインを指す。
...
デフォルトコンストラクタを除く[特殊メンバ関数](#SS_6_3_1)、
...
その場合、この原則が対象とする関数は、
[特殊メンバ関数](#SS_6_3_1)のすべてとなる。

このガイドラインに従って、コピーやムーブを実装する場合、

* [等価性のセマンティクス](#SS_6_17_1)
* [copyセマンティクス](#SS_6_17_2)
* [moveセマンティクス](#SS_6_17_3)
...
* 全ての[特殊メンバ関数](#SS_6_3_1)がデフォルトである。
* バーチャル関数や仮想継承を持たない。
* 基底クラスがある場合、基底クラスもトリビアルである。

である。その結果、トリビアル型とは、[トリビアルに破壊可能な型](#SS_6_3_3)となる。
...
「型が[トリビアル型](#SS_6_3_2)且つ[標準レイアウト型](#SS_6_3_4)であること」と
「型が[POD](#SS_6_3_6)であること」は等価であるため、C++20では、
...
このようなクラスは[POD](#SS_6_3_6)
(C++20では、[PODという用語は非推奨](https://cpprefjp.github.io/lang/cpp20/deprecate_pod.html)
となり、[トリビアル型](#SS_6_3_2)と[標準レイアウト型](#SS_6_3_4)に用語が分割された)とも呼ばれ、
C言語の構造体のレイアウトと互換性を持つことが一般的である。

ポリモーフィックなクラスは、
仮想関数呼び出しを行う(「[オーバーライドとオーバーロードの違い](#SS_6_3_10)」参照)
ためのメモリレイアウトが必要になる。
...
各クラスがvtblへのポインタを保持するため、このドキュメントで使用している[g++](#SS_6_19_1)では、
...
(Derived::g()はオーバーロードでもオーバーライドでもない(「[name-hiding](#SS_6_10_9)」参照))。
...
[ポリモーフィックなクラス](#SS_6_3_9)とは密接な関係を持つ。


下記のような[ポリモーフィックなクラス](#SS_6_3_9)に対しては、
...
一方で、下記のような非[ポリモーフィックなクラス](#SS_6_3_9)に対しては、
...
typeidは[RTTI](#SS_6_3_11)オブジェクトの型情報
([std::type_info](#SS_6_3_11_3))を実行時に取得するための演算子である。
dynamic_castとは違い、
typeidのオペランドは[ポリモーフィックなクラス](#SS_6_3_9)のインスタンスでなくても良い。
...
下記のような[ポリモーフィックなクラス](#SS_6_3_9)のインスタンスに関して、
...
[ポリモーフィックなクラス](#SS_6_3_9)のオブジェクトをオペランドとするtypeidの実行は、
...
一方、非[ポリモーフィックなクラス](#SS_6_3_9)のオブジェクトをオペランドとするtypeidのオペランド式は、
...
このドキュメントのコードのコンパイルに使用している[g++](#SS_6_19_1)/[clang++](#SS_6_19_2)では、
そのような関数は、`abi::__cxa_demangle`である。

`std::type_info::name()`と`abi::__cxa_demangle`を利用して、
オブジェクトの[被修飾型](#SS_6_18_10)名をstd::stringオブジェクトとして取り出す関数とその使用例を以下に示す。
...
constexpr定数がif文のオカレンスになる場合、[constexpr if文](#SS_6_11_10)することで、
[ill-formed](#SS_6_18_5)を使用した場合分けが可能になる。
...
1. 以下のいずれかに該当する式であること  
   ...
   - constexpr関数呼び出し
   - sizeof演算子の適用結果
   - typeid演算子の適用結果(式の値が[ポリモーフィックなクラス](#SS_6_3_9)である場合を除く)
...
[リスト初期化](#SS_6_5_1)による初期化方法がC++における初期化を統一的に扱えるように設計された概念を指さす。

### 初期化子リストコンストラクタ <a id="SS_6_5_3"></a>
初期化子リストコンストラクタ([リスト初期化](#SS_6_5_1)用のコンストラクタ)とは、
{}による[リスト初期化](#SS_6_5_1)をサポートするためのコンストラクタである。
...
* [NSDMI](#SS_6_5_6_1)
* [初期化子リストでの初期化](#SS_6_5_6_2)
* [コンストラクタ内での非静的なメンバ変数の初期値の代入](#SS_6_5_6_3)

同一変数に対して、
「[NSDMI](#SS_6_5_6_1)」と「[初期化子リストでの初期化](#SS_6_5_6_2)」
...
オブジェクトの所有権やその移動、
std::unique_ptr、std::move()、[rvalue](#SS_6_14_3)の関係を解説する。
...
下記に示した上記クラスの単体テストにより、
オブジェクトの所有権やその移動、共有、
std::shared_ptr、std::move()、[rvalue](#SS_6_14_3)の関係を解説する。
...
* prvalue(「[rvalue](#SS_6_14_3)」参照)のライフタイム
...
このような状態のリファレンスを[danglingリファレンス](#SS_6_15_5)と呼ぶ。
同様に、このような状態のポインタを[danglingポインタ](#SS_6_15_6)と呼ぶ。
...
スライシングが起こった場合、そうならないことが問題である(「[等価性のセマンティクス](#SS_6_17_1)」参照)。
...
[ユーザ定義リテラル演算子](#SS_6_6_6_1)により定義されたリテラルを指す。
...
### ==演算子 <a id="SS_6_6_7"></a>
クラスの==演算子の実装方法には、
[メンバ==演算子](#SS_6_6_7_1)、[非メンバ==演算子](#SS_6_6_7_2)の2つの方法がある。

#### メンバ==演算子 <a id="SS_6_6_7_1"></a>
メンバ==演算子には、[非メンバ==演算子](#SS_6_6_7_2)に比べ、下記のようなメリットがある。
...
非メンバ==演算子には、[メンバ==演算子](#SS_6_6_7_1)に比べ、下記のようなメリットがある。
...
* [暗黙の型変換](#SS_6_13_1)を利用した以下に示すようなシンプルな記述ができる場合がある。
...
比較演算子とは、[==演算子](--)の他に、!=、 <=、>、>= <、>を指す。
C++20から導入された[<=>演算子](#SS_6_6_8_3)の定義により、すべてが定義される。
...
なお、std::rel_opsはC++20から導入された[<=>演算子](#SS_6_6_8_3)により不要になったため、
非推奨とされた。

#### std::tuppleを使用した比較演算子の実装方法 <a id="SS_6_6_8_2"></a>
クラスのメンバが多い場合、[==演算子](#SS_6_6_7)で示したような方法は、
...
「[std::tuppleを使用した比較演算子の実装方法](#SS_6_6_8_2)」
...
三方比較演算子とは[<=>演算子](#SS_6_6_8_3)を指す。

#### spaceship operator <a id="SS_6_6_8_5"></a>
spaceship operatorとは[<=>演算子](#SS_6_6_8_3)を指す。
...
co_returnは通常[co_await](#SS_6_8_1_1)と同時に使われることが多い。
...
[co_await](#SS_6_8_1_1)、co_returnの例でみたように、
...
* [ジェネリックラムダ](#SS_6_11_4)とは、C++11のラムダ式を拡張して、
...
* **`std::unique_ptr`** 
   はダイナミックにアロケートされた[オブジェクトの排他所有](#SS_6_5_7_1)を表すために用いられる。  
* **`std::shared_ptr`** 
   はダイナミックにアロケート[オブジェクトの共有所有](#SS_6_5_7_2)を表現、管理するために用いられる。   
* **[std::weak_ptr](#SS_6_5_7_4)**
   は`std::shared_ptr`と組み合わせて使用される補助的なスマートポインタである。
   参照カウントに影響を与えず、[オブジェクトの循環所有](#SS_6_5_7_3)よるメモリリークを防ぐために用いられる。
   std::weak_ptr`はリソースへの弱い参照を保持し、リソースの有効性を確認する際に使用される。  
* `std::auto_ptr`はC++11以前に導入された初期のスマートポインタであるが、
   異常な[copyセマンティクス](#SS_6_17_2)を持つため、多くの誤用を生み出し、
...
このドキュメントでのルックアップとは[name lookup](#SS_6_10_2)を指す。
...
1. ToString()呼び出しの引数xの型Xが名前空間NS_LUで定義されているため、
   ToStringを探索する名前空間にNS_LUを組み入れる(「[関連名前空間](#SS_6_10_6)」参照)
...
2. 1の後、テンプレートパラメータを展開した関数内で、
   [関連名前空間](#SS_6_10_6)の宣言も含めたname lookupを行う。
...
1. TypeName()呼び出しの引数xの型Xが名前空間NS_TPLUで宣言されているため、
   NS_TPLUをTypeNameを探索する[関連名前空間](#SS_6_10_6)にする。
...
3. TypeNameは関数テンプレートであるためtwo phase lookupが以下のように行われる。
    ...
    2. TypeName内でのToType(t)の呼び出しに対しては、2nd name lookupが行われる。
       ...
       tの型がNS_TPLU::Xであるため[関連名前空間](#SS_6_10_6)となったNS_TPLUがname lookupの対象となるが、
       グローバル名前空間内のToTypeは、
       NS_TPLU内でTypeNameより前に宣言されたtemplate<> ToTypeによって[name-hiding](#SS_6_10_9)が起こり、
...
1. NS_TPLUを名前空間と関連付けする
   (引数の型がintなのでNS_TPLUは[関連名前空間](#SS_6_10_6)とならず、NS_TPLUを明示する必要がある)。
2. TypeName()呼び出しより前方で宣言されたNS_TPLUの中からTypeNameを見つける。
3. TypeNameは関数テンプレートであるためtwo phase lookupが以下のように行われる。
    1. TypeName内でのHeader(int{})の呼び出しは、1st name lookupにより、
       前例と同じ理由で、Header(long)の宣言と関連付けられる。
    2. TypeName内でのToType(t)の呼び出しに対しては、2nd name lookupが行われる。
       tの型がintであるためNS_TPLUは[関連名前空間](#SS_6_10_6)とならず、通常のname lookupと同様に
...
サンプルコードを[g++](#SS_6_19_1)と[clang++](#SS_6_19_2)でコンパイルしている)。
...
によるとこの動作は未定義のようだが、
[g++](#SS_6_19_1)/[clang++](#SS_6_19_2)両方ともこのコードを警告なしでコンパイルする)。
...
* TypeNameの中で行われる2nd name lookupの対象となる宣言は下記の和集合である。
    ...
    * TypeName呼び出しより前方にある[関連名前空間](#SS_6_10_6)内の宣言

この認識に基づくNS_TPLU2::Yに対するグローバルなTypeName内でのtwo phase name lookupは、

1. TypeName内に1st name lookupの対象がないため何もしない。
2. TypeName内の2nd name lookupに使用される[関連名前空間](#SS_6_10_6)NS_TPLU2は、
...
このテストはエラーとならなければならないが、[g++](#SS_6_19_1)ではパスしてしまう。
2nd name lookupのロジックにバグがあるようである。

有難いことに、[clang++](#SS_6_19_2)では仕様通りこのテストはエラーとなり、
...
通常はその略語である[ADL](#SS_6_10_5)と呼ばれる。

### ADL <a id="SS_6_10_5"></a>
ADLとは、関数の実引数の型が宣言されている名前空間(これを[関連名前空間](#SS_6_10_6)と呼ぶ)内の宣言が、
その関数の[name lookup](#SS_6_10_2)の対象になることである。
...
すでに述べたようにこれをADLと呼び、この場合のNS_ADLを[関連名前空間](#SS_6_10_6)と呼ぶ。
...
[ADL](#SS_6_10_5)(実引数依存探索)によってname lookupの対象になった宣言を含む名前空間のことである。
...
これにより、意図しない[name lookup](#SS_6_10_2)を回避することができるため、可読性と安全性が向上する。
...
* クラスの内部で定義された、
* 名前空間スコープでの通常の[name lookup](#SS_6_10_2)できず、[ADL](#SS_6_10_5)のみでname lookupできる
...
[name lookup](#SS_6_10_2)の対象外になる」現象一般をを指す通称である
...
下記のように[using宣言](#SS_6_10_14)を使用することで、
...
修飾の副作用として「[two phase name lookup](#SS_6_10_3)」の例で示したような
[ADL](#SS_6_10_5)を利用した高度な静的ディスパッチが使用できなくなるが、
...
ダイヤモンド継承は、
[仮想継承](#SS_6_10_11)(virtual inheritance)を使ったものと、使わないものに分類できる。

[仮想継承](#SS_6_10_11)を使わないダイヤモンド継承のコードを以下に示す。
...
次に示すのは、[仮想継承](#SS_6_10_11)を使用したダイヤモンド継承の例である。
...
(が、[仮想継承](#SS_6_10_11)による別の問題が発生する)。
...
仮想継承は、[ダイヤモンド継承](#SS_6_10_10)の基底クラスのインスタンスを、
...
[ダイヤモンド継承](#SS_6_10_10)での基底クラスのコンストラクタ呼び出しを一度にするために存在する。
...
仮想基底(クラス)とは、[仮想継承](#SS_6_10_11)の基底クラス指す。

### ドミナンス <a id="SS_6_10_13"></a>
[ドミナンス(Dominance、支配性)](https://en.wikipedia.org/wiki/Dominance_(C%2B%2B))とは、
「探索対称の名前が継承の中にも存在するような場合の[name lookup](#SS_6_10_2)の仕様の一部」
を指す慣用句である。

以下に

* [ダイヤモンド継承を含まない場合](#SS_6_10_13_1)
* [ダイヤモンド継承かつそれが仮想継承でない場合](#SS_6_10_13_2)
* [ダイヤモンド継承かつそれが仮想継承である場合](#SS_6_10_13_3)

のドミナンスについてのコードを例示する。

この例で示したように、[ダイヤモンド継承](#SS_6_10_10)を通常の継承で行うか、
[仮想継承](#SS_6_10_11)で行うかでは結果が全く異なるため、注意が必要である。
...
この[name lookup](#SS_6_10_2)については、[name-hiding](#SS_6_10_9)で説明した通りである。
...
これまでと同様にDerived_0のドミナンスによりBase::fは[name-hiding](#SS_6_10_9)されることになる。
この時、Derived_0、Derived_1がBaseから[仮想継承](#SS_6_10_11)した効果により、
この継承ヒエラルキーの中でBaseは１つのみ存在することになるため、
Derived_1により導入されたBase::fも併せて[name-hiding](#SS_6_10_9)される。
...
[using宣言](#SS_6_10_14)よりも危険であり、また、
下記のように[name-hiding](#SS_6_10_9)された識別子の導入には効果がない。
...
「テンプレートのパラメータ置き換えに失敗した([ill-formed](#SS_6_18_5)になった)際に、
即時にコンパイルエラーとはせず、置き換えに失敗したテンプレートを
[name lookup](#SS_6_10_2)の候補から除外する」
という言語機能である。
...
テンプレートの型推論ガイド([CTAD(Class Template Argument Deduction)](#SS_6_11_7))は、
C++17で導入された機能である。この機能により、
クラステンプレートのインスタンス化時にテンプレート引数を明示的に指定せず、
引数から自動的に型を推論できるようになる。型推論ガイドを使用することで、
コードの可読性と簡潔性が向上する。

型推論ガイドがない場合、[クラステンプレートのテンプレート引数の型推論](#SS_6_11_5)は限定的であり、
...
CTAD(Class Template Argument Deduction)とは、[テンプレートの型推論ガイド](#SS_6_11_6)のことである。
...
decltypeはオペランドに[expression](#SS_6_14_1)を取り、その型を算出する機能である。
...
(「[autoパラメータによる関数テンプレートの簡易定義](#SS_6_11_11)」を参照)。
...
C++14から導入された[関数の戻り値型auto](#SS_6_12_6)と似た、
...
- [暗黙の型変換](#SS_6_13_1)
- [暗黙の型変換抑止](#SS_6_13_2)
- [explicit type operator()](#SS_6_13_3)
- [explicit(COND)](#SS_6_13_4)
...
といったセマンティクス的観点(「[シンタックス、セマンティクス](#SS_6_17)」参照)によるものである。
...
下記のコードのように[暗黙の型変換](#SS_6_13_1)が起こる。
...
C++03までは、[一様初期化](#SS_6_5_2)がサポートされていなかったため、
explicitは単一引数のコンストラクタに使用されることが一般的であった。

C++11からサポートされた[一様初期化](#SS_6_5_2)を下記のように使用することで、
暗黙の型変換を使用できる。
...
[rvalue修飾](#SS_6_14_4)を参照せよ。


### リファレンス修飾 <a id="SS_6_14_6"></a>
[rvalue修飾](#SS_6_14_4)と[lvalue修飾](#SS_6_14_5)とを併せて、リファレンス修飾と呼ぶ。


## リファレンス <a id="SS_6_15"></a>
ここでは、C++11から導入された

* [ユニバーサルリファレンス](#SS_6_15_1)
* [リファレンスcollapsing](#SS_6_15_4)
...
「[ユニバーサルリファレンス](#SS_6_15_1)」を参照せよ。

### perfect forwarding <a id="SS_6_15_3"></a>
perfect forwarding とは、引数の[rvalue](#SS_6_14_3)性や
[lvalue](#SS_6_14_2)性を損失することなく、
その引数を別の関数に転送する技術のことを指す。
通常は、[ユニバーサルリファレンス](#SS_6_15_1)である関数の仮引数をstd::forwardを用いて、
他の関数に渡すことで実現される。
...
このようなリファレンスにアクセスすると、[未定義動作](#SS_6_18_7)に繋がるに繋がる。
...
danglingポインタとは、[danglingリファレンス](#SS_6_15_5)と同じような状態になったポインタを指す。
...
### exception-unfriendly <a id="SS_6_16_5"></a>
以下のような関数  
...
の呼び出しでエクセプションがthrowされると、[未定義動作](#SS_6_18_7)や[未規定動作](#SS_6_18_8)が発生するため、
...
* クラスや関数がその名前から想起できる責務を持っている
    * 「[単一責任の原則(SRP)](#SS_2_1)」を満たしている
    * [Accessor](#SS_3_4)を実装する関数の名前は、GetXxxやSetXxxになっている
    * コンテナクラスのメンバ関数beginやendは、
      そのクラスが保持する値集合の先頭や最後尾の次を指すイテレータを返す
    * 等

* 「[等価性のセマンティクス](#SS_6_17_1)」を守ってる
* 「[copyセマンティクス](#SS_6_17_2)」を守ってる
* 「[moveセマンティクス](#SS_6_17_3)」を守っている
...
組み込みの==やオーバーロード[==演算子](#SS_6_6_7)のこのような性質をここでは「等価性のセマンティクス」と呼ぶ。
...
文字列リテラルからstd::stringへの[暗黙の型変換](#SS_6_13_1)が起こるために成立する。
...
従って、これらのオブジェクトに対して[等価性のセマンティクス](#SS_6_17_1)
...
下記の単体テストから明らかな通り、
「[等価性のセマンティクス](#SS_6_17_1)」で示した最後の例も、copyセマンティクスを満たしていない。
...
* a == bがtrueの時に、c = std::move(a)が行われた場合、
  ...
  そのリソースはcに移動している」ことが一般的である(「[rvalue](#SS_6_14_3)」参照)。

* [no-fail保証](#SS_6_16_1)をする(noexceptと宣言し、エクセプションをthrowしない)。
...
4. 効率性  
   ...
   これは、リソースの複製を避けることで達成される(「[moveセマンティクス](#SS_6_17_3)」参照)。

5. デフォルト実装  
   ムーブ代入演算子が明示的に定義されていない場合でも、
   クラスが一定の条件(例: ムーブ不可能なメンバが存在しないこと)を満たしていれば、
   コンパイラがデフォルトの実装(「[特殊メンバ関数](#SS_6_3_1)」参照)を生成する。
...
3. 正しいセマンティクス  
   コピー代入によって代入元のオブジェクトが変更されてはならない(「[copyセマンティクス](#SS_6_17_2)」参照)。
...
[alignas](#SS_6_18_1_2)、
[alignof](#SS_6_18_1_1)によりコンパイラの標準的な方法でアライメントのコントロールできるようになった。
...
それに対し、C++14までは、演算子のオペランド部分式の評価順序は[未規定動作](#SS_6_18_8)であった。
...
* [well-formed](#SS_6_18_6)(適格)とはプログラムが全ての構文規則・診断対象の意味規則・
  単一定義規則を満たすことである。
...
| 種類            |定義                                                               | 例                               | 結果                           |
|-----------------|-------------------------------------------------------------------|----------------------------------|--------------------------------|
|[未定義動作](#SS_6_18_7)|C++標準が全く保証しない動作                                        | ゼロ除算、配列範囲外アクセス     | 予測不能(クラッシュなど)       |
|[未規定動作](#SS_6_18_8)|C++標準が動作を定めていないが、いくつかの選択肢が許容されている動作| `int8_t` に収まらない値のキャスト| 実装依存(異なるが合理的な動作) |
...
見た目が類似する[修飾付き関数呼び出し](#SS_6_10_7)とは無関係である。

### one-definition rule <a id="SS_6_18_11"></a>
「[ODR](#SS_6_18_12)」を参照せよ。
...
[初期化子リストコンストラクタ](#SS_6_5_3)の呼び出しでオブジェクトの初期化を行うことで、
このような問題を回避できる。
...
(ただし、「[Immutable](#SS_3_6)」の観点からは、QuadraticEquation::Set()がない方が良い)。
...
<!-- ./md/sample_code.md -->
# Sample Code <a id="SS_7"></a>
## C++ <a id="SS_7_1"></a>
### example/dynamic_memory_allocation/malloc_ut.cpp <a id="SS_7_1_1"></a>
```cpp
...
### example/dynamic_memory_allocation/mpool_variable.h <a id="SS_7_1_2"></a>
```cpp
          1 #pragma once
          2 #include <cassert>
          3 #include <cstdint>
