...
&emsp;&emsp;&emsp; [ジェネリックラムダ](template_meta_programming.md#SS_4_6_2)  
&emsp;&emsp;&emsp; [クラステンプレートと継承の再帰構造](template_meta_programming.md#SS_4_6_3)  
&emsp;&emsp;&emsp; [constexpr if文](template_meta_programming.md#SS_4_6_4)  
&emsp;&emsp;&emsp; [意図しないname lookupの防止](template_meta_programming.md#SS_4_6_5)  
&emsp;&emsp;&emsp; [Nstd::Type2Strの開発](template_meta_programming.md#SS_4_6_6)  
&emsp;&emsp;&emsp; [静的な文字列オブジェクト](template_meta_programming.md#SS_4_6_7)  
&emsp;&emsp;&emsp; [関数型をテンプレートパラメータで使う](template_meta_programming.md#SS_4_6_8)  
...
このドキュメントで使用している[g++](term_explanation.md#SS_6_9_1)ではこのコードはコンパイルでき、
動作も問題ないように思われるが、[clang++](term_explanation.md#SS_6_9_2)では以下のようなエラーが発生し、コンパイルできない。
...
* operator<<(std::ostream& os, App::Ints_t const& ints)
  ...
  (「[割れ窓理論](term_explanation.md#SS_6_12_1)」参照)になってしまうかもしれない)。
...
コードクローンや、[車輪の再発明](term_explanation.md#SS_6_12_2)による開発効率の低下につながることがある。
...
* [SFINAE](term_explanation.md#SS_6_4_7)
...
* std::enable_if\<true>::typeは[well-formed](term_explanation.md#SS_6_10_9)
* std::enable_if\<false>::typeは[ill-formed](term_explanation.md#SS_6_10_8)
...
std::enable_ifのこの特性と後述する[SFINAE](term_explanation.md#SS_6_4_7)により、
...
[SFINAE](term_explanation.md#SS_6_4_7)を使用した関数テンプレートis_void_sfinae_fの実装は以下のようになる。
...
[SFINAE](term_explanation.md#SS_6_4_7)を使用したクラステンプレートis_void_sfinae_sの実装は以下のようになる。
...
「[is_void_ena_sの実装](template_meta_programming.md#SS_4_3_2_5)」でその例を示す。


#### is_void_ena_sの実装 <a id="SS_4_3_2_5"></a>
[std::enable_if](template_meta_programming.md#SS_4_3_1_4)による[SFINAE](term_explanation.md#SS_6_4_7)とクラステンプレートの特殊化を使用した
...
この例では、「[is_void_sfinae_sの実装](template_meta_programming.md#SS_4_3_2_4)」の
...
で行っている。
std::enable_ifの値パラメータis_void_f\<T>()は、「[is_void_fの実装](template_meta_programming.md#SS_4_3_2_1)」で示したものである。

単体テストは、「[is_void_sfinae_sの実装](template_meta_programming.md#SS_4_3_2_4)」で示したものとほぼ同様で、以下のようになる。
...
std::conditionalの値パラメータis_void_f\<T>()は、「[is_void_fの実装](template_meta_programming.md#SS_4_3_2_1)」で示したものである。
この例では、SFINAEもクラステンプレートの特殊化も使用していないが、
下記単体テストからわかる通り、「[is_void_sfinae_sの実装](template_meta_programming.md#SS_4_3_2_4)」と同じ機能を備えている。
...
「[is_same_vの実装](template_meta_programming.md#SS_4_3_3_2)」と同様に単純であるため、解説は不要だろう。 単体テストは以下のようになる。
...
「[is_void_sfinae_sの実装](template_meta_programming.md#SS_4_3_2_4)」とほぼ同様であるため、解説は不要だろう。 
単体テストは以下のようになる。
...
「[IsSameSomeOfの実装](template_meta_programming.md#SS_4_3_3_7)」のコードパターンとほぼ同様であるため、解説は不要だろうが、
...
上記を抜粋した下記のコードは「縮小型変換を発生さる{}による初期化は[ill-formed](term_explanation.md#SS_6_10_8)になる」
ことをSFINAEに利用している。
...
「[exists_void_func_sfinae_sの実装](template_meta_programming.md#SS_4_3_5_2)」
...
* [ill-formed](term_explanation.md#SS_6_10_8)ならばill-formedになる
* [well-formed](term_explanation.md#SS_6_10_9)ならvoidを生成する
...
の戻り型が配列型の[rvalue](term_explanation.md#SS_6_5_3)である"int (&&) [3]"となり、
...
そのオブジェクトが[lvalue](term_explanation.md#SS_6_5_2)(この例ではint (&)[3])であれば、
...
「[exists_void_func_sfinae_fの実装](template_meta_programming.md#SS_4_3_5_1)」と同様のパターンを使用したので解説は不要だろう。
...
「[exists_begin/exsits_endの実装](template_meta_programming.md#SS_4_3_5_4)」と「[exists_put_to_as_memberの実装](template_meta_programming.md#SS_4_3_5_6)」
...
「[IsRangeの実装](template_meta_programming.md#SS_4_3_5_5)」に影響されて、一旦このように実装したが、先に書いた通り、
...
(「[is_same_templの実装](template_meta_programming.md#SS_4_3_3_6)」参照)を使用し下記のように書ける。
...
「[メタ関数のテクニック](template_meta_programming.md#SS_4_3)」で紹介したSFINAEにより、この問題は下記のように対処できる。
...
実際、現在使用中の[g++](term_explanation.md#SS_6_9_1)ではこのコードはコンパイルできず、
...
[ユニバーサルリファレンス](term_explanation.md#SS_6_6_1)を使って下記のように書き直した。
...
この原因は、
「関数が受け取った[rvalue](term_explanation.md#SS_6_5_3)リファレンスは、
その関数から別の関数に受け渡される時に[lvalue](term_explanation.md#SS_6_5_2)リファレンスとして扱われる」からである。
...
(T const&&はconstな[rvalue](term_explanation.md#SS_6_5_3)リファレンスである)、
ユニバーサルリファレンスが[lvalue](term_explanation.md#SS_6_5_2)リファレンスであった場合は、
...
なお、ユニバーサルリファレンスは、[リファレンスcollapsing](term_explanation.md#SS_6_6_4)の一機能としても理解できる。
...
このような場合、[SFINAE](term_explanation.md#SS_6_4_7)によるオーバーロードが必須であったが、
...
* [ジェネリックすぎるテンプレートを書かない](template_meta_programming.md#SS_4_6_5_1)
* [ADLが本当に必要でない限り名前を修飾する](template_meta_programming.md#SS_4_6_5_2)
* [ADL Firewallを使う](template_meta_programming.md#SS_4_6_5_3)
...
[ジェネリックラムダ](template_meta_programming.md#SS_4_6_2)を使えばよい。
...
残念なことにこの関数は非標準であるため、
それを使っているNstd::Inner\_::demangleは[g++](term_explanation.md#SS_6_9_1)/[clang++](term_explanation.md#SS_6_9_2)
でなければコンパイルできないだろう。
...
「[静的な文字列オブジェクト](template_meta_programming.md#SS_4_6_7)」で見たように、
...
* [ADL](term_explanation.md#SS_6_4_5)を利用しない場合、テンプレートで使う識別子は名前空間名やthis->等で修飾する
  (「[意図しないname lookupの防止](template_meta_programming.md#SS_4_6_5)」参照)。
...
* テンプレートのインターフェースではないが、実装の都合上ヘッダファイルに記述する定義は、
  ...
  外部から参照しない(「[is_void_sfinae_fの実装](template_meta_programming.md#SS_4_3_2_3)」参照)。
...
* ユニバーサルリファレンス引数を他の関数に渡すのであれば、std::forwardを使う
  (「[ユニバーサルリファレンス](term_explanation.md#SS_6_6_1)」、「[ユニバーサルリファレンスとstd::forward](template_meta_programming.md#SS_4_6_1)」参照)。
...
* テンプレートに関数型オブジェクトを渡す場合、リファレンスの付け忘れに気を付ける
  (「[関数型をテンプレートパラメータで使う](template_meta_programming.md#SS_4_6_8)」
  参照)。

* 意図しないテンプレートパラメータによるインスタンス化の防止や、
  コンパイルエラーを解読しやすくするために、
  適切にstatic_assert(「[exists_begin/exsits_endの実装](template_meta_programming.md#SS_4_3_5_4)」
  参照)を使う。
