...
(この実装は長いため、全体は巻末の"[example/dynamic_memory_allocation/malloc_ut.cpp](sample_code.md#SS_7_1_1)"に掲載する)。
...
[フリースタンディング環境](term_explanation.md#SS_6_10_20)では、sbrkのようなシステムコールは存在しないため、
...
* 複数個のメモリプールを統合的に扱う[インターフェースクラス](term_explanation.md#SS_6_1_13)MPool
* MPoolを基底クラスとし、固定長メモリブロックを管理するクラステンプレートMPoolFixed

によって実装することにする。

まずは、MPoolを下記に示す
(「[ファイル位置を静的に保持したエクセプションクラスの開発](template_meta_programming.md#SS_4_6_7_4)」参照)。
...
[一様初期化](term_explanation.md#SS_6_2_7)で行った方が良いが、
...
コンパイラに[g++](term_explanation.md#SS_6_9_1)や[clang++](term_explanation.md#SS_6_9_2)を使っている場合、
