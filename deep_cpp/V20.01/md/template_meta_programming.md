...
&emsp;&emsp; [ログ取得ライブラリの開発2](template_meta_programming.md#SS_4_5)  
&emsp;&emsp; [その他のテンプレートテクニック](template_meta_programming.md#SS_4_6)  
&emsp;&emsp;&emsp; [ユニバーサルリファレンスとstd::forward](template_meta_programming.md#SS_4_6_1)  
&emsp;&emsp;&emsp; [ジェネリックラムダによる関数内での関数テンプレートの定義](template_meta_programming.md#SS_4_6_2)  
&emsp;&emsp;&emsp; [クラステンプレートと継承の再帰構造](template_meta_programming.md#SS_4_6_3)  
&emsp;&emsp;&emsp; [意図しないname lookupの防止](template_meta_programming.md#SS_4_6_4)  
&emsp;&emsp;&emsp; [Nstd::Type2Strの開発](template_meta_programming.md#SS_4_6_5)  
&emsp;&emsp;&emsp; [静的な文字列オブジェクト](template_meta_programming.md#SS_4_6_6)  
&emsp;&emsp;&emsp; [関数型をテンプレートパラメータで使う](template_meta_programming.md#SS_4_6_7)  

&emsp;&emsp; [注意点まとめ](template_meta_programming.md#SS_4_7)  
...
* 可変長引数を持つ
* [算術型](term_explanation.md#SS_6_1_1)の引数と戻り値を持つ
* すべての引数の和を返す
...
Logging::Logger::set_innerでのApp::operator<<の[name lookup](term_explanation.md#SS_6_4_2)について考えてみることにする。
...
その認識とは、「テンプレート内の識別子の[name lookup](term_explanation.md#SS_6_4_2)は、
それがインスタンス化される時に行われる」というものであり、これにより
「 Logging::Loggerのname lookupは単体テスト内で行われる。
それはApp::operator<<宣言後であるためコンパイルできる」と考えることができるが、
[two phase name lookup](term_explanation.md#SS_6_4_3)で行われるプロセスと反するため誤りである。
...
[ADL](term_explanation.md#SS_6_4_5)(実引数依存探索)が働く。
また、Logging::Logger::set_inner(x)はテンプレートであるため、[two phase name lookup](term_explanation.md#SS_6_4_3)
が使用される。
...
instsの[関連名前空間](term_explanation.md#SS_6_4_6)もstdであり、Appではない。
その結果App::operator<<は発見できず、このようなエラーになった。

LOGGERからApp::operator<<を使う場合の単体テストは下記のようになるが、
[ADL](term_explanation.md#SS_6_4_5)によってLogging::Logger::set_inner(ints)内に導入される名前空間はstdのみであり、
前記単体テスト同様にコンパイルできない。
...
「[ADL](term_explanation.md#SS_6_4_5)によってLogging::Logger::set_inner(ints)内に導入される名前空間はstdである」
...
上記コードは[two phase name lookup](term_explanation.md#SS_6_4_3)等の効果により、想定通りコンパイルできるが、
...
このドキュメントで使用している[g++](term_explanation.md#SS_6_11_1)ではこのコードはコンパイルでき、
動作も問題ないように思われるが、[clang++](term_explanation.md#SS_6_11_2)では以下のようなエラーが発生し、コンパイルできない。
...
この理由は「[two phase name lookup](term_explanation.md#SS_6_4_3)」の後半で詳しく解説したので、ここでは繰り返さないが、
...
* operator<<(std::ostream& os, App::Ints_t const& ints)
  ...
  (「[割れ窓理論](term_explanation.md#SS_6_14_1)」参照)になってしまうかもしれない)。
...
これまで述べてきたようにApp::Inst_tの[関連名前空間](term_explanation.md#SS_6_4_6)がAppではなく、stdになってしまうからである。

これを回避するためにはその原因を取り払えばよく、
つまり、App::Inst_tの[関連名前空間](term_explanation.md#SS_6_4_6)がAppになるようにすればよい。
...
* using宣言によりstd::vectorのすべてのコンストラクタをApp::Ints_tに導入
  (「[継承コンストラクタ](term_explanation.md#SS_6_2_3)」参照)

としているため、エイリアスで宣言されたInts_tと等価である。
C++03では、[継承コンストラクタ](term_explanation.md#SS_6_2_3)が使えなかったため、
上記のような構造体を定義するためには、
...
コードクローンや、[車輪の再発明](term_explanation.md#SS_6_14_2)による開発効率の低下につながることがある。
...
これを初期化するためには初期化子リスト
(「[初期化子リストコンストラクタ](term_explanation.md#SS_6_2_2)」、
「[一様初期化](term_explanation.md#SS_6_2_7)」参照)を用いるのが良い。
...
* [SFINAE](term_explanation.md#SS_6_5_1)
...
* std::enable_if\<true>::typeは[well-formed](term_explanation.md#SS_6_12_8)
* std::enable_if\<false>::typeは[ill-formed](term_explanation.md#SS_6_12_7)
...
std::enable_ifのこの特性と後述する[SFINAE](term_explanation.md#SS_6_5_1)により、
...
[SFINAE](term_explanation.md#SS_6_5_1)を使用した関数テンプレートis_void_sfinae_fの実装は以下のようになる。
...
であるため、Tがvoidの時のみ[name lookup](term_explanation.md#SS_6_4_2)の対象になる。
...
であるため、Tが非voidの時のみ[name lookup](term_explanation.md#SS_6_4_2)の対象になる。
...
[SFINAE](term_explanation.md#SS_6_5_1)を使用したクラステンプレートis_void_sfinae_sの実装は以下のようになる。
...
is_void_sfinae_sの特殊化が[name lookup](term_explanation.md#SS_6_4_2)の対象の中に見つからなかった場合、
これが使われる。
...
[std::enable_if](template_meta_programming.md#SS_4_3_1_4)による[SFINAE](term_explanation.md#SS_6_5_1)とクラステンプレートの特殊化を使用した
...
|[same_as](template_meta_programming.md#SS_4_3_3_6)                |[コンセプト](term_explanation.md#SS_6_5_2)よるis_same_sfinae_sと同一の機能      |
...
「[is_same_v](template_meta_programming.md#SS_4_3_3_2)の実装」と同様に単純であるため、解説は不要だろう。 単体テストは以下のようになる。
...
「[is_void_sfinae_s](template_meta_programming.md#SS_4_3_2_4)の実装」とほぼ同様であるため、解説は不要だろう。 
単体テストは以下のようになる。
...
[SFINAE](term_explanation.md#SS_6_5_1)による[is_same_sfinae_s](template_meta_programming.md#SS_4_3_3_5)の難解なコードを[コンセプト](term_explanation.md#SS_6_5_2)
...
「[is_same_s](template_meta_programming.md#SS_4_3_3_3)」で紹介した特殊化のテクニックを下記のように使用することができる。
...
* 第1パラメータが第2パラメータ以降で指定された型のどれかと一致する
  SameAsSomeOfという名前の[コンセプト](term_explanation.md#SS_6_5_2)を[畳み込み式](term_explanation.md#SS_6_5_3)を使用し定義する
...
Usが1つだった場合、SameAsSomeOfは処理をstd::same_as(「[same_as](template_meta_programming.md#SS_4_3_3_6)」参照)に委譲する。
...
OneOfは、[IsSameSomeOf](template_meta_programming.md#SS_4_3_3_8)同様の機能を持つコンセプトである。
OneOfの実装にはシンプルに記述するための[畳み込み式](term_explanation.md#SS_6_5_3)を使用した。
...
|--------------------------------------|---------------------------------------------|
|[AreConvertible](template_meta_programming.md#SS_4_3_4_1)                 |クラステンプレートの特殊化                   |
|[ConvertibleToAll](template_meta_programming.md#SS_4_3_4_3)               |AreConvertibleをコンセプトへ                 |
|[AreConvertibleWithoutNarrowConv](template_meta_programming.md#SS_4_3_4_2)|SFINAEとクラステンプレートの特殊化           |
|[ConvertibleWithoutNarrowing](template_meta_programming.md#SS_4_3_4_4)    |AreConvertibleWithoutNarrowConvをコンセプトへ|
...
「[IsSameSomeOf](template_meta_programming.md#SS_4_3_3_8)の実装」のコードパターンとほぼ同様であるため、解説は不要だろうが、
...
上記を抜粋した下記のコードは「縮小型変換を発生さる{}による初期化は[ill-formed](term_explanation.md#SS_6_12_7)になる」
ことをSFINAEに利用している。
...
* [AreConvertibleWithoutNarrowConv](template_meta_programming.md#SS_4_3_4_2)と同様の機能を持つ
* [ConvertibleToAll](template_meta_programming.md#SS_4_3_4_3)と同様構造を持つ
...
|メタ関数名                              |メタ関数の目的                                    |
|----------------------------------------|--------------------------------------------------|
|[exists_void_func_sfinae_f](template_meta_programming.md#SS_4_3_5_1)        |メンバ関数void func()を持つかどうかの判断         |
|[exists_void_func_sfinae_s](template_meta_programming.md#SS_4_3_5_2)        |同上                                              |
|[exists_void_func_sfinae_s2](template_meta_programming.md#SS_4_3_5_3)       |同上                                              |
|[exists_void_func_concept](template_meta_programming.md#SS_4_3_5_4)         |同上。コンセプトによるSFINAEの回避                |
...
|メタ関数名                            |メタ関数の目的                                                     |
|--------------------------------------|-------------------------------------------------------------------|
|[exists_begin/exsits_end](template_meta_programming.md#SS_4_3_5_5)        |SFINAEを使用したstd::begin(T)/std::end(T)が存在するか否かの診断    |
|[Array](template_meta_programming.md#SS_4_3_5_7)                          |型が配列である制約を行うためのコンセプト                           |
|[Beginable/Endable](template_meta_programming.md#SS_4_3_5_8)              |[コンセプト](term_explanation.md#SS_6_5_2)を使用したexists_begin/exsits_endを単純化した例   |
|[IsRange](template_meta_programming.md#SS_4_3_5_6)                        |exists_begin/exsits_endを使し、範囲forのオペランドになれるか?の判断|
|[Ranged](template_meta_programming.md#SS_4_3_5_9)                         |機能はIsRangeと同一だが、[コンセプト](term_explanation.md#SS_6_5_2)を使用しSFINAEの回避     |
|[Container](template_meta_programming.md#SS_4_3_5_10)                      |Ranged且つ!Arrayをコンテナと便宜的に決めつける                     |
...
|メタ関数名                            |メタ関数の目的                                         |
|--------------------------------------|-------------------------------------------------------|
|[exists_put_to_as_member](template_meta_programming.md#SS_4_3_5_11)        |std::ostream::operator<<(T)が存在するか否かの診断      |
|[exists_put_to_as_non_member](template_meta_programming.md#SS_4_3_5_12)    |operator<<(std::ostream&, T)が存在するか否かの診断     |
|[ExistsPutTo](template_meta_programming.md#SS_4_3_5_13)                    |std::ostream& << Tができるかどうかの診断               |
|[Printable](template_meta_programming.md#SS_4_3_5_14)                      |std::ostream& << Tができるかどうか制約コンセプト       |
...
[exists_void_func_sfinae_s](template_meta_programming.md#SS_4_3_5_2)や[exists_void_func_sfinae_s2](template_meta_programming.md#SS_4_3_5_3)
...
以下に示すように、C++20から導入された[コンセプト](term_explanation.md#SS_6_5_2)はこのような問題の軽減につながる。
...
「[exists_void_func_sfinae_s](template_meta_programming.md#SS_4_3_5_2)」
...
* [ill-formed](term_explanation.md#SS_6_12_7)ならばill-formedになる
* [well-formed](term_explanation.md#SS_6_12_8)ならvoidを生成する
...
の戻り型が配列型の[rvalue](term_explanation.md#SS_6_7_3)である"int (&&) [3]"となり、
...
そのオブジェクトが[lvalue](term_explanation.md#SS_6_7_2)(この例ではint (&)[3])であれば、
...
コンセプトを使用し、[exists_begin/exsits_end](template_meta_programming.md#SS_4_3_5_5)をリファクタリングした例を以下に示す。
...
すでにみたようにRangedは[exists_begin/exsits_end](template_meta_programming.md#SS_4_3_5_5)の醜いコードを使用しないことで、
...
「[exists_void_func_sfinae_f](template_meta_programming.md#SS_4_3_5_1)の実装」と同様のパターンを使用したので解説は不要だろう。
...
「[exists_begin/exsits_end](template_meta_programming.md#SS_4_3_5_5)や[exists_put_to_as_member](template_meta_programming.md#SS_4_3_5_11)の実装」
...
「[IsRange](template_meta_programming.md#SS_4_3_5_6)の実装」に影響されて、一旦このように実装したが、先に書いた通り、
...
* [SFINAE](term_explanation.md#SS_6_5_1)を利用した[ExistsPutTo](template_meta_programming.md#SS_4_3_5_13)は複雑で醜いため、リファクタリングする。
...
これ以降は、[ExistsPutTo](template_meta_programming.md#SS_4_3_5_13)ではなくPrintableを使用する。
...
この特殊化のテンプレートパラメータの制約にはすでに開発したコンセプト[Container](template_meta_programming.md#SS_4_3_5_10)を使用する。
...
縮小型変換が起こるか否かの場合分けは、
コンセプト[ConvertibleWithoutNarrowing](template_meta_programming.md#SS_4_3_4_4)を使用したSFINAEで実現させることができる。
...
このような記述が必要になる。
一方で[コンセプト](term_explanation.md#SS_6_5_2)を使用したC++20スタイルのSFINAEの可読性の高さを実感できただろう。
...
(「[is_same_templ](template_meta_programming.md#SS_4_3_3_7)」参照)を使用し下記のように書ける。
...
値を型に変換する[std::integral_constant](template_meta_programming.md#SS_4_3_1_2)を使用し、この問題を解決できる。
...
ここでも、すでにみてきた[コンセプト](term_explanation.md#SS_6_5_2)による制約によりこの問題に対処する。
...
実際、現在使用中の[g++](term_explanation.md#SS_6_11_1)ではこのコードはコンパイルできず、
...
[ユニバーサルリファレンス](term_explanation.md#SS_6_8_1)を使って下記のように書き直した。
...
この原因は、
「関数が受け取った[rvalue](term_explanation.md#SS_6_7_3)リファレンスは、
その関数から別の関数に受け渡される時に[lvalue](term_explanation.md#SS_6_7_2)リファレンスとして扱われる」からである。
...
[畳み込み式](term_explanation.md#SS_6_5_3)を使用した下記の実装の方がより明確である。
...
(T const&&はconstな[rvalue](term_explanation.md#SS_6_7_3)リファレンスである)、
ユニバーサルリファレンスが[lvalue](term_explanation.md#SS_6_7_2)リファレンスであった場合は、
...
なお、ユニバーサルリファレンスは、[リファレンスcollapsing](term_explanation.md#SS_6_8_4)の一機能としても理解できる。
...
この記法は[ジェネリックラムダ](term_explanation.md#SS_6_5_4)と呼ばれる。
...
ここで示した関数テンプレートは、デザインパターン[Visitor](design_pattern.md#SS_3_20)の例であり、
...
このコードパターンについては、
「[CRTP(curiously recurring template pattern)](design_pattern.md#SS_3_21)」で説明している。
...
[ADL](term_explanation.md#SS_6_4_5)(実引数依存探索)により、Appもis_equalの[name lookup](term_explanation.md#SS_6_4_2)の対象になるからである。
...
* [ジェネリックすぎるテンプレートを書かない](template_meta_programming.md#SS_4_6_4_1)
* [ADLが本当に必要でない限り名前を修飾する](template_meta_programming.md#SS_4_6_4_2)
* [ADL Firewallを使う](template_meta_programming.md#SS_4_6_4_3)
...
[ジェネリックラムダによる関数内での関数テンプレートの定義](template_meta_programming.md#SS_4_6_2)を行えばよい。
...
* [std::enable_if](template_meta_programming.md#SS_4_3_1_4)や[コンセプト](term_explanation.md#SS_6_5_2)等を使用してテンプレートに適用できる型を制約する
...
([two phase name lookup](term_explanation.md#SS_6_4_3)の一回目のname lookupでfがバインドされるため)。
...
[ADL](term_explanation.md#SS_6_4_5)や[two phase name lookup](term_explanation.md#SS_6_4_3)が絡む場合ならなおさらである。

こういう意図しない[name lookup](term_explanation.md#SS_6_4_2)に備えるためには、
...
XY型オブジェクトを引数にした関数呼び出しによる[関連名前空間](term_explanation.md#SS_6_4_6)は、
...
等によるコンパイルエラーとの戦いである。
また、これをクリアしても[two phase name lookup](term_explanation.md#SS_6_4_3)や[ADL](term_explanation.md#SS_6_4_5)が次の関門になる。
...
残念なことにこの関数は非標準であるため、
それを使っているNstd::Inner\_::demangleは[g++](term_explanation.md#SS_6_11_1)/[clang++](term_explanation.md#SS_6_11_2)
でなければコンパイルできないだろう。
...
[非explitなコンストラクタによる暗黙の型変換](term_explanation.md#SS_6_2_5)
...
「[静的な文字列オブジェクト](template_meta_programming.md#SS_4_6_6)」で見たように、
...
「[RAII(scoped guard)](design_pattern.md#SS_3_9)でも示したScopedGuardの実装を下記する。
...
* [name lookup](term_explanation.md#SS_6_4_2)には複雑なルールが適用されるため、非直感的なバインドが行われる場合がある。
  従って、テンプレートライブラリの開発には単体テストは必須である。

* 使用しているコンパイラが[two phase name lookup](term_explanation.md#SS_6_4_3)をサポートしているか否かに気を付ける。
  それがオプションである場合は、two phase name lookupを活性化させる。

* 関数型マクロはそれ以外に実装方法がない時のみに使用する
  。

* 可変長引数を持つ関数の実装には[パラメータパック](template_meta_programming.md#SS_4_1_3)を使う。

* 処理速度や関数のリターンの型に影響する場合があるため、
  パラメータパックの処理の順番に気を付ける(「[前から演算するパラメータパック](template_meta_programming.md#SS_4_1_3_2)」参照)。

* [ADL](term_explanation.md#SS_6_4_5)を利用しない場合、テンプレートで使う識別子は名前空間名やthis->等で修飾する
  (「[意図しないname lookupの防止](template_meta_programming.md#SS_4_6_4)」参照)。
...
* テンプレートのインターフェースではないが、実装の都合上ヘッダファイルに記述する定義は、
  ...
  外部から参照しない(「[is_void_sfinae_f](template_meta_programming.md#SS_4_3_2_3)の実装」参照)。
...
* ユニバーサルリファレンス引数を他の関数に渡すのであれば、std::forwardを使う
  (「[ユニバーサルリファレンス](term_explanation.md#SS_6_8_1)」、「[ユニバーサルリファレンスとstd::forward](template_meta_programming.md#SS_4_6_1)」参照)。

* 関数テンプレートとその特殊化はソースコード上なるべく近い位置で定義する
  (「[two phase name lookup](term_explanation.md#SS_6_4_3)」参照)。

* [two phase name lookup](term_explanation.md#SS_6_4_3)により意図しない副作用が発生する可能性があるため、
...
* 関数テンプレートのオーバーロードと特殊化の[name lookup](term_explanation.md#SS_6_4_2)の優先度に気を付ける。
...
* ユニバーサルリファレンスを持つ関数テンプレートをオーバーロードしない。
  「[ユニバーサルリファレンスとstd::forward](template_meta_programming.md#SS_4_6_1)」で述べたように、
...
* テンプレートに関数型オブジェクトを渡す場合、リファレンスの付け忘れに気を付ける
  (「[関数型をテンプレートパラメータで使う](template_meta_programming.md#SS_4_6_7)」
  参照)。

* 意図しないテンプレートパラメータによるインスタンス化の防止や、
  コンパイルエラーを解読しやすくするために、適切にstatic_assert使うことは重要であるが、
  static_assertによるテンプレートパラメータの制約よりも、[コンセプト](term_explanation.md#SS_6_5_2)による制約を優先する。
